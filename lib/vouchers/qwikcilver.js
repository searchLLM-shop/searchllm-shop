...
// mask the real error).
async function qcRequest(method, path, body, { _retried = false, corruptToken = false, corruptSignature = false } = {}) {
  const apiBase = process.env.QWIKCILVER_API_BASE || "https://sandbox.woohoo.in/rest";
  const url = `${apiBase}${path}`;
  // corruptToken/corruptSignature (2026-09-14): deliberate-failure injection
  // for the UAT test kit's #17 "Invalid Token set" / #4 "Signature/nonce
  // invalid" cases — never used by issueVoucher() or any real code path,
  // only by testOrderCustom() below via an explicit opt-in flag.
  const token = corruptToken ? "invalid.test.token" : await getBearerToken();
  ...
  let signature = buildSignature({ method, url, bodyString, clientSecret: process.env.QWIKCILVER_CLIENT_SECRET });
  if (corruptSignature) signature = signature.slice(0, -4) + "dead"; // still hex-shaped, just wrong
  ...
  if (resp.status === 401 && !_retried && !corruptToken && !corruptSignature) {
...
// Flexible sibling to testOrder() (2026-09-14), built specifically to
// exercise the failure-scenario rows of Qwikcilver's UAT test-case sheet
// (#6, #12, #14, #16-20, #27) that testOrder()'s fixed qty=1/payments=svc/
// single-product shape can't reach. Every field defaults to exactly what
// testOrder() sends, so a call with no overrides is behaviourally
// identical — overrides are additive, never a second code path to drift
// out of sync with the real (working) request shape.
//
// options:
//   testSkuKey    - looked up via TEST_SKUS (same as testOrder)
//   sku           - raw SKU string, takes precedence over testSkuKey
//                    (lets a real BRAND_SKU_MAP-style "Any" SKU be used
//                    for e.g. #16 invalid-payment-method, where the SKU
//                    itself is incidental to what's being tested)
//   denomination  - default 1000
//   qty           - default 1 (set to 5+ for #6/#19)
//   paymentCode   - default "svc" (set to e.g. "TESTING" for #16)
//   refno         - default auto-generated; pass a REUSED value for #18
//   telephone     - overrides billingFromEnv()'s phone on both billing
//                    and address, for #20's blacklisted-number case
//   products      - full raw override for the `products` array, takes
//                    precedence over sku/denomination/qty (for #27's
//                    two-SKUs-in-one-order case)
//   corruptToken / corruptSignature - forwarded to qcRequest, for #17/#4
export async function testOrderCustom(options = {}) {
  const {
    testSkuKey,
    sku: skuOverride,
    denomination = 1000,
  ...
  const sku = skuOverride || TEST_SKUS[testSkuKey];
  if (!productsOverride && !sku) {
    return { ok: false, reason: `Pass either "sku", "testSkuKey" (one of: ${Object.keys(TEST_SKUS).join(", ")}), or "products".` };
  }
  ...
  if (telephone) billing.telephone = telephone;

  const refno = refnoOverride || `sllm_test_${Date.now()}`;
  const products = productsOverride || [{ sku, price: denomination, qty, currency: 356, giftMessage: "", theme: "" }];
  // payments[0].amount is the ORDER TOTAL (sum of price*qty across every
  // product line), not a single line's unit price — testOrder() never hit
  // this because it always sends qty:1/one product, where the two happen
  // to be equal. Found 2026-09-14 running #6 (qty=5) and #27 (two SKUs)
  // for real: both got rejected with "Payment amount is not matching the
  // uploaded value" until this was computed correctly.
  const amountTotal = products.reduce((sum, p) => sum + Number(p.price) * Number(p.qty || 1), 0);
  const body = {
    billing,
    address: { ...billing, billToThis: true },
    payments: [{ code: paymentCode, amount: amountTotal }],
    refno,
    successUrl: "https://searchllm.shop/",
    failureUrl: "https://searchllm.shop/",
    products,
  };
  const debugBodyString = JSON.stringify(sortKeysDeep(body));

  let resp;
  try {
    resp = await qcRequest("POST", "/v3/orders", body, { corruptToken, corruptSignature });
  } catch (err) {
    return { ok: false, reason: `Qwikcilver request failed: ${String(err?.message || err)}`, debug: { bodyString: debugBodyString } };
  }
  const data = await resp.json().catch(() => null);
  return { ok: resp.status === 201 || resp.status === 202, status: resp.status, refno, data, debug: { ...resp._qcDebug, bodyLength: debugBodyString.length } };
}
