...
//   4. (Found 2026-09-08, via a real signed POST failing with
//      oauth_problem=signature_invalid while GETs kept succeeding.) The
//      signature must be computed over the EXACT bytes sent as the
...
//      string for both the signature and the actual request body.
//
...
export async function testOrder(testSkuKey, denomination = 100) {
  ...
  return { ok: resp.status === 201, status: resp.status, refno, data, debug: { bodyString: debugBodyString, bodyLength: debugBodyString.length } };
}
