...
async function qcRequest(method, path, body, { _retried = false } = {}) {
  ...
  resp._qcDebug = { url, signature, bodyString, dateAtClient };
...
export async function testOrder(testSkuKey, denomination = 100) {
  ...
  const body = {
    ...
    // successUrl/failureUrl: present in Qwikcilver's own confirmed-working
    // Order request (2026-09-09) — presumably for a hosted/redirect
    // checkout flow we don't use (deliveryMode:"API" returns the card
    // directly in this response instead), but included to match their
    // real example exactly since this whole path has been signature_
    // invalid despite every other verification passing.
...
export async function issueVoucher({ brand, denomination, redemptionId }) {
  ...
  // own Postman sample request omits both), so they're left out. See
  // testOrder()'s matching comment for successUrl/failureUrl/giftMessage/
  // numeric currency — all corrected 2026-09-09 against a real,
  // confirmed-working Order request Qwikcilver sent us directly.
  const body = {
    billing,
    address: { ...billing, billToThis: true },
    payments: [{ code: "svc", amount: denomination }],
