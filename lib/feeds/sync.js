...
export async function runFullSync() {
  ...
  // Parallel to productFeedUrls by index (2026-09-09, added for Myntra
  // alongside the existing Shopsy feed) — one vCommission publisher account
  // runs a separate campaign_id per merchant, and campaign_id has to travel
  // with its own feed URL, not be a single global value every feed shares.
  // A shorter list than productFeedUrls (or unset entirely) falls back to
  // fetchVcommissionProductFeed's own DEFAULT_CAMPAIGN_ID for the missing
  // entries — existing single-feed setups keep working unchanged.
  const productCampaignIds = (process.env.VCOMMISSION_CAMPAIGN_IDS || "")
      ...
      while (Date.now() - startedAt < PRODUCT_TIME_BUDGET_MS) {
        ...
        const { listings, nextOffset, done } = await fetchVcommissionProductFeed(
          productFeedUrls[feedIdx],
          {
            ...
            offset: byteOffset,
            campaignId: productCampaignIds[feedIdx],
          }
        );
  ...
  // vCommission PRODUCT feeds (e.g. Shopsy, Myntra) are hosted CSV files,
  // not part of the Publisher API. Set VCOMMISSION_PRODUCT_FEED_URLS to a
  // comma-separated list of feed URLs; each sync run pulls one, advancing
  // by cursor so a large catalogue imports across runs instead of blowing
  // memory in one go. VCOMMISSION_CAMPAIGN_IDS is the parallel comma-
  // separated list of that feed's own vCommission campaign_id (each
  // merchant is its own campaign under the one publisher account) — see
  // lib/feeds/vcommissionProducts.js's multi-campaign note.
