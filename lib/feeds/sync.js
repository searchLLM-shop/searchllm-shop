...
export async function runFullSync() {
  ...
  if (productFeedUrls.length) {
    try {
      ...
      const cursor = await getFeedCursor("vcommission_product_cursor");
      const index = cursor % productFeedUrls.length;

      const listings = await fetchVcommissionProductFeed(productFeedUrls[index], {
        limit: SYNC_LIMIT_PER_NETWORK,
        skipIds,
        // Marketplace feeds carry a long tail of very cheap, unbranded items
        // that make poor recommendations. A price floor filters the worst of
        // it; raise MIN_PRODUCT_PRICE if the queue still looks noisy.
      ...
      await setFeedCursor("vcommission_product_cursor", (index + 1) % productFeedUrls.length);
      await logSyncRun({ network: "vCommission", status: "success", productsSeen: listings.length, newListings: inserted, updatedListings: updated });
      results.push({ network: "vCommission products", status: "success", productsSeen: listings.length, newListings: inserted, updatedListings: updated, feed: `${index + 1}/${productFeedUrls.length}` });
    } catch (err) {
