...
import { upsertFeedListing, logSyncRun, getExistingExternalIds, getFeedCursor, setFeedCursor } from "@/lib/db";
...
export async function runFullSync() {
  ...
  if (process.env.AWIN_DATAFEED_API_KEY) {
    try {
      ...
      for (const listing of listings) {
        if (!listing.externalId || !listing.networkLink) continue;
        const { isNew } = await upsertFeedListing("Awin", listing);
        if (isNew) newListings += 1;
        else updatedListings += 1;
      }

      // Advance the cursor so the next run downloads the next feed.
      if (feedCount > 0) {
      ...
      await logSyncRun({ network: "Awin", status: "error", errorMessage: err.message });
      results.push({ network: "Awin", status: "error", error: err.message });
    }
  }
