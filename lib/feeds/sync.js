...
import { upsertFeedListing, logSyncRun, getExistingExternalIds } from "@/lib/db";
...
export async function runFullSync() {
  ...
  // Each network only runs if its credentials are actually configured, so
  // an unconfigured or half-configured network can't crash the whole sync.
  const results = [];

  if (process.env.AWIN_DATAFEED_API_KEY) {
    const awinSkip = await getExistingExternalIds("Awin");
    results.push(await syncNetwork("Awin", () => fetchAwinFeed(process.env.AWIN_DATAFEED_API_KEY, { limit: SYNC_LIMIT_PER_NETWORK, skipIds: awinSkip })));
  }
  ...
  if (process.env.VCOMMISSION_API_KEY && process.env.VCOMMISSION_NETWORK_ID) {
