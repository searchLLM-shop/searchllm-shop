...
export async function runFullSync() {
  ...
  if (productFeedUrls.length) {
    try {
      ...
      const { listings, nextOffset, done, rangeSupported, diag } = await fetchVcommissionProductFeed(
      ...
      await logSyncRun({
        ...
        // When nothing comes back, record why — otherwise a zero is unreadable.
        errorMessage: listings.length === 0 && diag ? JSON.stringify(diag).slice(0, 400) : null,
      });
      results.push({
        network: "vCommission Products",
        status: "success",
