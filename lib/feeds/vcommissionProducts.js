...
export async function fetchVcommissionProductFeed(feedUrl, opts = {}) {
  ...
  if (!text || !text.trim()) return { listings: [], nextOffset: 0, done: true };
  ...
  if (!rangeHonoured) {
    return { listings, nextOffset: 0, done: false, rangeSupported: false };
  }
  const done = totalSize > 0 && consumedTo >= totalSize;
  return { listings, nextOffset: done ? 0 : consumedTo, done, rangeSupported: true };
}
