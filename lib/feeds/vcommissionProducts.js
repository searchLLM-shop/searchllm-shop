...
export async function fetchVcommissionProductFeed(feedUrl, opts = {}) {
  ...
  const requiredIdx = {
    ...
    tracking_url: header.indexOf("tracking_url"),
  };
  function isAlignedAt(text, nlIndex) {
    ...
    const pid = rows[0][requiredIdx.product_id];
    const turl = rows[0][requiredIdx.tracking_url];
    return Boolean(pid && turl && String(turl).startsWith("http"));
  }
