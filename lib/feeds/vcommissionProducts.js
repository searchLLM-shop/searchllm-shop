...
// This vendor's feed schema has already drifted once (see the two
// conflicting "real columns" comments above — samples said image_url, the
// live file said image_link) and there's no reason to trust it's settled.
// Rather than hardcode one column name and silently lose every product
// image the next time it drifts, try the candidates that have actually
// appeared across their sample and live files, in order. imageUrl ends up
// null (never a broken guess) only if none of them are present — the same
// safe fallback as before, just resilient to which name is current.
// /api/admin/feeddiag reports the real column names if this ever needs
...
//
// MULTI-CAMPAIGN (2026-09-09): one vCommission publisher account (PUB_ID)
// runs several campaigns, one per merchant — Shopsy was campaign 11008;
// Myntra is a separate campaign (10882, confirmed via the tracking link
// https://track.vcommission.com/click?campaign_id=10882&pub_id=130649
// the user provided). campaign_id is NOT a property of a product row — it's
// a property of WHICH FEED the row came from — so it's threaded through as
// a parameter from lib/feeds/sync.js's per-feed-URL loop (see
// VCOMMISSION_CAMPAIGN_IDS there), not read from a single global env var.
// The module-level default below only covers the single-feed case (no
// VCOMMISSION_CAMPAIGN_IDS configured) for backward compatibility.
const DEFAULT_CAMPAIGN_ID = process.env.VCOMMISSION_CAMPAIGN_ID || "11008";
...
export function parseProductRow(raw, campaignId) {
  ...
  const link = buildTrackedLink(productUrl, campaignId);
  if (!link) return null;
  ...
  // Was hardcoded to "Shopsy" — silently correct only as long as Shopsy was
  // the only feed. With Myntra (and any future merchant) added, an empty
  // brand column would have mislabelled every one of THEIR products as
  // Shopsy too. Derived from the product's own domain instead, so it's
  // correct for whichever merchant's feed this row actually came from.
  ...
  return {
    // Prefixed with campaignId (2026-09-09): externalId is only unique
    // within (network, external_id) — see lib/db.js's upsertFeedListing —
    // and every vCommission product feed shares the literal network value
    // "vCommission" regardless of merchant. Without this prefix, Myntra
    // and Shopsy products that happen to reuse the same numeric feed id
    // would silently overwrite each other.
    externalId: `${campaignId || DEFAULT_CAMPAIGN_ID}-${id}`,
 ...
 * @param {string} opts.campaignId this feed's vCommission campaign — see the
 *                                 multi-campaign note above buildTrackedLink
 */
export function parseProductFeed(text, { limit = 500, skipIds = new Set(), minPrice = 0, campaignId } = {}) {
  ...
  for (let i = 1; i < rows.length && listings.length < limit; i++) {
    ...
    const listing = parseProductRow(obj, campaignId);
...
export async function fetchVcommissionProductFeed(feedUrl, opts = {}) {
  const { limit = 150, skipIds = new Set(), minPrice = 0, offset = 0, chunkSize = 3 * 1024 * 1024, campaignId } = opts;
