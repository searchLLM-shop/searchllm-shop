...
// Extracts a display domain ("nike.com") from a URL, used to show shoppers the
// destination of an affiliate link rather than hiding it behind a redirect.
...
export function parseAwinProduct(raw) {
  ...
  return {
    ...
    networkLink: link,
    // Real Awin feeds expose several image sizes; prefer the merchant's own
    // full-size image and fall back through Awin's copies.
    imageUrl: raw.merchant_image_url || raw.aw_image_url || raw.large_image || null,
    // Shown to the shopper so they can see where the link goes before clicking.
    merchantDomain: domainFrom(raw.merchant_deep_link || raw.data_feed_url) || null,
    discount: null,
...
export async function fetchAwinFeed(dataFeedApiKey, { limit = 500, skipIds = new Set(), cursor = 0 } = {}) {
  ...
  // The feed list carries a "Primary Region" column (GB, US, IN, ...) for the
  // advertiser. Every product from this feed inherits it, so we can avoid
  // showing a UK-only merchant's GBP prices to a shopper in another country.
  const regionKey = Object.keys(feed).find((k) => k.trim().toLowerCase().includes("region"));
  const feedRegion = regionKey ? String(feed[regionKey] || "").trim().toUpperCase() : "";
  const regions = feedRegion ? [feedRegion] : null;
  ...
  try {
    ...
    for (const raw of products) {
      ...
      if (normalized && !skipIds.has(normalized.externalId)) {
