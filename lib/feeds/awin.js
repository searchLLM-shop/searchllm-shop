...
export async function fetchAwinFeed(dataFeedApiKey, { limit = 500, skipIds = new Set() } = {}) {
  ...
  for (const feed of feeds) {
    ...
    try {
      ...
      // Once we've hit the overall limit, stop opening more advertiser
      // feeds — no point downloading and decompressing more product files
      // we'll only throw away, which is what spiked memory before.
      if (listings.length >= limit) break;
    } catch (err) {
