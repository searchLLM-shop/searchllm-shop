...
export async function fetchAwinFeed(dataFeedApiKey, { limit = 500, skipIds = new Set(), cursor = 0 } = {}) {
  ...
  // Keep only feeds we can actually use (have a URL, and joined if a
  // membership column is present).
  const usableFeeds = allFeeds.filter((feed) => {
    ...
    if (statusKey && feed[statusKey] && !/active|joined|approved/i.test(feed[statusKey])) return false;
    return true;
  });
