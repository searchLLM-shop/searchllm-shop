...
async function fetchAndParseFeedCsv(url, maxRows = 0) {
  ...
  let text = isGzip ? gunzipSync(buf).toString("utf8") : buf.toString("utf8");
  ...
  // Release the big decompressed string ASAP — on serverless, holding the
  // full feed text plus the parsed rows plus the objects at once is what
  // spikes memory and gets the instance killed.
  text = null;
  const objects = rowsToObjects(maxRows > 0 ? rows.slice(0, maxRows + 1) : rows);
  return objects;
}

export async function fetchAwinFeed(dataFeedApiKey, { limit = 500, skipIds = new Set() } = {}) {
  if (!dataFeedApiKey) {
  ...
  // Hard cap on how many advertiser feeds we open in a single run. The list
  // can contain many feeds; downloading and decompressing all of them in one
  // serverless invocation accumulates memory until the instance is killed
  // (this was the real cause of the "out of memory" sync failures — many
  // productdata.awin.com calls in one request, not one big file). One feed
  // per run is enough to keep products flowing; more runs pull more feeds.
  ...
  for (const feed of feeds) {
    ...
    if (feedsProcessed >= MAX_FEEDS_PER_RUN) break;
    ...
    try {
      ...
      for (const raw of products) {
        ...
        // Skip products already imported in a previous run so each sync
        // advances to new items instead of re-reading the same first N.
        if (normalized && !skipIds.has(normalized.externalId)) listings.push(normalized);
      }
      // If this feed yielded NO new products (fully imported already),
      // don't count it against the per-run cap — let the loop move on to
      // the next advertiser feed so later runs reach fresh feeds instead
      // of getting stuck re-scanning an exhausted one.
      if (listings.length === before) feedsProcessed -= 1;
      if (listings.length >= limit) break;
    } catch (err) {
