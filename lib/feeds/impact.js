...
function parseImpactItem(raw) {
  ...
  return {
    ...
    // Impact's catalog items expose ImageUrl; Url is the merchant product page.
    imageUrl: raw.ImageUrl || raw.ImageURL || null,
    merchantDomain: (() => {
      try { return new URL(raw.Url).hostname.replace(/^www\./, ""); } catch { return null; }
    })(),
    discount: null,
...
export async function fetchImpactFeed(accountSid, authToken, { limit = 500 } = {}) {
  ...
  for (const catalog of Array.isArray(catalogs) ? catalogs : []) {
    ...
    try {
      ...
      while (listings.length < limit && page <= maxPages) {
