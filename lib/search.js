...
// The two arms of retrieval, each playing to a measured strength
// (2026-07-30 benchmark): Brave uniquely returns a `discussions` block —
// real forum threads with the question and top reply — which is the best
// available signal for "what do owners actually say". Serper returns
// Google's index with Indian price-aggregator pages (91mobiles, smartprix,
// Croma) whose snippets carry live rupee figures. Neither substitutes for
// the other, so both fire, in parallel, on different queries.
export function contextSearchProvider() {
  const p = (process.env.SEARCH_CONTEXT_PROVIDER || "brave").toLowerCase();
  return ["brave", "serper", "tavily", "serplify"].includes(p) ? p : "brave";
}

export function priceSearchProvider() {
  const p = (process.env.SEARCH_PRICE_PROVIDER || process.env.SEARCH_PROVIDER || "serper").toLowerCase();
  return ["brave", "serper", "tavily", "serplify"].includes(p) ? p : "serper";
}
