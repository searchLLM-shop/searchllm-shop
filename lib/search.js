...
export function searchProvider() {
  ...
  return ["brave", "serper", "tavily"].includes(p) ? p : "brave";
}
...
const ADAPTERS = { brave: braveAdapter, serper: serperAdapter, tavily: tavilyAdapter };

// The only function callers need. Never throws.
export async function webSearch(query, count = 4, country = "IN") {
...
export async function webSearchDiag(query, provider, count = 4, country = "IN") {
  ...
  try {
    ...
    return { ok: true, provider, parsed: results, raw };
  } catch (err) {
