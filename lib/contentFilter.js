...
//
// POLICY (product decision, 2026-07-22, contraband category added 2026-09):
// we do not research, answer, or match products in these categories, each
...
//   - contraband, smuggled goods, terrorism-related items or merchandise
//     (including anything tied to an organisation banned/proscribed under
//     Indian law), and propaganda material unlawful to possess or sell
...
const MINOR_LISTING_TERMS = [
  ...
  // "baby"/"babies" added 2026-09-18: real gap found chasing a "women red
  // dress" mismatch — several of the wrongly-shortlisted listings were
  // titled "Baby Girls Above Knee Party Dress...", and this list didn't
  // catch "baby" at all.
  "infant", "toddler", "baby", "babies", "school",
];
...
// Same term list, as a single POSIX regex for Postgres's ~* operator
// (\y is Postgres's word-boundary metacharacter, the SQL-side equivalent
// of containsTerm's \W boundary above) — lib/db.js's findCandidateListings
// uses this to exclude minor-audience listings AT THE SQL LEVEL, not just
// after JS-side scoring. Real gap found 2026-09-18: findTopMatchingListings
// already excluded them post-fetch, but findCandidateListings only ever
// fetches the top 200 candidates by search rank — on a query like "women
// red dress for a party", this catalog's kids'-dress titles rank so
// densely that genuine women's dresses never survived into that top-200
// window at all, so the JS-side exclusion had nothing left to rescue.
export const MINOR_SQL_REGEX = `\\y(${MINOR_LISTING_TERMS.join("|")})\\y`;

// Query-side adult-AUDIENCE test (2026-09-18), a broader sibling to
// hasAdultContext above: "women red dress for a party" carries no sexual
// language at all, so hasAdultContext never fired, but the same underlying
// problem hasAdultContext was built for on 2026-07-22 showed up anyway —
// every one of the top-scoring candidates was a girls'/baby party dress,
// none a genuine women's dress, despite the catalog holding comparable
// numbers of both (confirmed via direct count, not a catalog gap). The
// mechanical scorer has no concept of "audience"; it only counts
// overlapping words, and this catalog's kids'-dress titles happen to
// repeat generic occasion words ("Party", "Above Knee") that a terser
// "Women A-line Red Dress" title doesn't. Kept deliberately narrow — just
// the unambiguous adult-clothing audience words, not every gendered term.
const ADULT_AUDIENCE_TERMS = [
  "women", "woman", "womens", "women's", "ladies", "lady",
  "men", "mens", "men's", "gents", "gentleman", "gentlemen",
];
export function mentionsAdultAudience(text) {
  const t = (text || "").toLowerCase();
  return ADULT_AUDIENCE_TERMS.some((term) => containsTerm(t, term));
}
...
const RESTRICTED_CATEGORIES = [
  ...
  },
  {
    name: "contraband",
    message:
      "We don't cover contraband, smuggled goods, terrorism-related items, or unlawful propaganda material. Try a different shopping question.",
    terms: [
      // Terrorism-related items and banned-organisation merchandise — named
      // examples (ISIS, Al Qaeda, Taliban) are illustrative, not exhaustive;
      // the message and the general-unlawfulness catch-all in the Terms of
      // Use cover organisations not individually listed here.
      "terrorist merchandise", "terrorism propaganda", "isis flag", "isis merchandise",
      "al qaeda merchandise", "taliban merchandise", "jihadi merchandise",
      "suicide vest", "suicide bomb vest", "banned organisation merchandise",
      "banned organization merchandise", "nazi flag", "nazi memorabilia", "third reich memorabilia",
      // Unlawful propaganda material
      "extremist propaganda", "hate propaganda", "genocide propaganda",
      // Contraband, smuggled goods, illegal wildlife trade — deliberately
      // phrased as compound terms (not bare "ivory"/"tiger skin") to avoid
      // catching legitimate faux/synthetic decorative goods and prints.
      "smuggled goods", "contraband goods", "black market goods",
      "illegal wildlife trade", "poached wildlife", "ivory trade",
    ],
  },
];
