...
export const LOYALTY = {
  ...
  // creditClickPoints/syncLoyaltyLedger in lib/db.js) — but WHETHER they
  // keep accumulating past VOUCHER_UNLOCK_POINTS depends on plan, see below.
  // Click points are still credited once per product per day, because
  // uncapped identical clicks would be a self-service points mint.
  POINTS: {
  ...
  // Redesigned 2026-08-25 (reinstated by explicit direction after a brief
  // uncapped period): a non-Plus registered user's points STOP accumulating
  // at this total (search + click + purchase, combined) — every credit
  // function checks this and simply doesn't insert past it. Reaching it is
  // what a registered user needs to see before paying for Plus; paying is
  // what lifts the ceiling — a Plus member's points are UNCAPPED past this
  // point (see creditSearchPoints/creditClickPoints/syncLoyaltyLedger).
  // The same number doubles as the redemption eligibility threshold (see
  // requestRedemption) — reaching it and being Plus are both required to
  // actually redeem a voucher.
