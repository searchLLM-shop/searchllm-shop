...
// Loyalty programme economics. Purchase points were REMOVED entirely
// (2026-08-25, explicit direction) — points now come from search and click
// activity ONLY. Why: purchase points were never honestly verifiable at
// scale. Only Awin's conversion feed is confirmed working; vCommission's
// conversions endpoint was never verified against a real response; Amazon
// Associates has no self-serve conversion API at all, so an Amazon
// purchase could never have earned a point regardless. Rather than pay
// out points on a claim we can't actually confirm for most of the
// business, purchases now earn nothing — simple, and honest about what we
// can and can't verify. This is also a genuine privacy simplification:
// rewards no longer require linking a purchase to an account at all (see
// PRIVACY_POLICY §6A). Conversion tracking itself (network_clicks,
// commission reporting, the Plus usage-cycle reset on a completed
// purchase) is UNCHANGED and continues independently of this — those
// exist for revenue reporting and the fair-use cycle, never for points.
// INR only where currency still applies: mixing currencies into one points
// balance would be dishonest arithmetic.
export const LOYALTY = {
  POINT_VALUE_INR: 1,        // 1 point = ₹1 of voucher value

  // --- Flat points, identical rate for every user (free or Plus) ---
  // Guest points are a day-expiring teaser (never stored — computed live,
  // vanish at midnight unless the person registers). Registered points earn
  // at these flat rates regardless of plan (see creditSearchPoints/
  // creditClickPoints in lib/db.js) — but WHETHER they keep accumulating
  // past VOUCHER_UNLOCK_POINTS depends on plan, see below. Click points are
  // still credited once per product per day, because uncapped identical
  // clicks would be a self-service points mint.
  POINTS: {
    ...
    // No PURCHASE entry — see the file header above for why.
  },

  // Reinstated 2026-08-25 by explicit direction: a non-Plus registered
  // user's points STOP accumulating at this total (search + click,
  // combined) — every credit function checks this and simply doesn't
  // insert past it. Reaching it is what a registered user needs to see
  // before paying for Plus; paying is what lifts the ceiling — a Plus
  // member's points are UNCAPPED past this point (see creditSearchPoints/
  // creditClickPoints). The same number doubles as the redemption
  // eligibility threshold (see requestRedemption) — reaching it and being
  // Plus are both required to actually redeem a voucher.
...
The rewards programme is optional and free to join for registered users. Points accrue at a flat rate, the same for every member whether or not you are on the Plus plan: a fixed number of points per research pick, and a fixed number of points per recommended-product link click (subject to the once-per-product-per-day rule we set; guest points expire the same day unless you register and claim them). Points carry no cash value and are not transferable. Purchases earn no points, of any amount — nothing about what you buy is linked to your rewards balance. For a free (non-Plus) member, points accrue up to the total shown in the Rewards tab (currently 250) and then pause — no further points are earned until you upgrade to Plus; earning then continues without expiry or limit. REDEEMING points into a voucher is, separately, a benefit of the paid Plus plan, in the fixed voucher denominations we list, at 1 point = ₹1 of voucher value. We may adjust earning rates, denominations and voucher options prospectively (never removing already-confirmed points), and may withhold or void points obtained through fraud, automated or manipulative searching, self-dealing, or abuse. The programme may be modified or discontinued with reasonable notice; confirmed points will remain redeemable for a wind-down period in that event.
