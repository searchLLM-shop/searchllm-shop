...
// pricing copy can never disagree (design 2026-07-30):
//
//   guest                        → 8 a day. Registering is the way out.
//   signed in, under 250 points  → no daily cap. Creating an account is
//                                  rewarded immediately and visibly.
//   signed in, 250 points hit    → 8 a day returns. They have taken the
//                                  full engagement reward; from here the
//                                  platform asks for something back.
//   Plus                         → no daily cap (the per-cycle allowance
//                                  in LOYALTY.GATE_LIMITS still applies).
//
// Note the interaction: at 5 points a pick, 250 points is ~50 picks, which
// is also the free per-cycle gate. So an uncapped registered user reaches
// both boundaries at roughly the same moment — by design, not by accident.
export function dailyPickLimit({ signedIn, plan, engagementPoints = 0, isAdmin = false }) {
  if (isAdmin) return -1;
  if (plan === "plus") return PLANS.plus.searches;
  if (!signedIn) return PLANS.free.searches;
  return engagementPoints >= LOYALTY.ENGAGEMENT_POINTS_LIFETIME_CAP
    ? PLANS.free.searches
    : -1;
}
...
export const PLANS = {
  free: {
    ...
    savedPicks: 20,
    price: 0,
    features: ["Unlimited picks a day when signed in (8 a day as a guest, and again after the 250-point rewards maximum)", "Top pick + alternatives", "Save up to 20 picks"],
  },
  plus: {
    name: "Plus",
    ...
    // said "₹500/month" while the rewards copy said "₹499/year".
    price: 500,        // INR
