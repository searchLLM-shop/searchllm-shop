...
const SYSTEM_PROMPT = `You generate search keywords for shopping listings.
...
- Use the PRODUCT TYPE the merchant sells (e.g. "whey protein", "handbag", "moisturiser"), not marketing or affiliate jargon
- NEVER include affiliate/campaign terms: cps, cpi, cpa, ecommerce, campaign, offer, india, affiliate
...
export async function generateKeywords(listings, { apiKey = process.env.ANTHROPIC_API_KEY } = {}) {
  ...
  for (let i = 0; i < listings.length; i += BATCH_SIZE) {
    ...
    try {
      const batchResults = await enrichBatch(batch, apiKey);
      for (const [id, kws] of batchResults) results.set(id, kws);
    } catch (err) {
      // Keep going — a failed batch shouldn't discard the ones that worked.
      console.error(`Keyword batch ${i / BATCH_SIZE + 1} failed:`, err.message);
