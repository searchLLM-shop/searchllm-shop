...
export async function pollConversions({ lookbackDays = 45 } = {}) {
  ...
  // Purchase points removed from the platform (2026-08-25, see lib/db.js's
  // syncLoyaltyLedger and the LOYALTY.POINTS comment in lib/constants.js)
  // — conversions are still recorded above (network_clicks) for revenue
  // reporting and the Plus usage-cycle reset, just no longer turned into
  // points, so this no longer calls syncLoyaltyLedger.
