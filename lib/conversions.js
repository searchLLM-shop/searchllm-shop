...
//   - vCommission (Trackier): p1 as the click-ID carrier is CONFIRMED by
//     vCommission (Ritika, 2026-07-22). The conversions endpoint comes from
//     their publisher API docs (api-docs.trackier.io/docs/perf-pub-api-docs)
//     via the VCOMMISSION_CONVERSIONS_URL env var. The exact field name
//     that echoes p1 in the report is the last unknown — the adapter reads
//     several candidates and ?diag=1 shows which one is real.

import { updateClickConversion, logSyncRun, syncLoyaltyLedger } from "@/lib/db";
...
// Plural /publishers/ per the official docs (the earlier singular
// /publisher/ guesses are why the first attempt 404'd — the campaigns API
// from the handover happens to use singular, the reports APIs use plural).
const VCOMMISSION_ENDPOINT_CANDIDATES = [
  "https://api.trackier.com/v2/publishers/conversions",
  "https://api.trackier.com/v2/publishers/reports/conversions",
  "https://api.trackier.com/v2/publishers/conversion-report",
  "https://api.trackier.com/v2/publishers/conversions/report",
...
async function fetchVcommissionConversions(lookbackDays) {
  ...
  // The documented auth is an X-Api-Key HEADER (the older campaigns API
  // used ?apiKey=). Send both — endpoints ignore what they don't read.
...
// The DOCUMENTED aggregate performance report (per the official docs,
// 2026-07-22): campaign-level clicks/conversions/payout/saleAmount. This is
// real revenue data available immediately — but it is AGGREGATE: no p1
// grouping exists on this endpoint, so it cannot attribute a sale to a
// click. Per-click matching needs the row-level Conversion Report endpoint
// from the same docs collection. Read-only; nothing is stored.
export async function fetchVcommissionPerformanceReport({ lookbackDays = 30 } = {}) {
  const apiKey = process.env.VCOMMISSION_API_KEY;
  if (!apiKey) return { skipped: "VCOMMISSION_API_KEY not set" };
  const end = new Date();
  const start = new Date(end.getTime() - lookbackDays * 86400000);
  const kpis = [
    "clicks", "grossClicks", "approvedConversions", "pendingConversions",
    "rejectedConversions", "payout", "pendingPayout", "saleAmount", "pendingSaleAmount",
  ].map((k) => `kpi[]=${k}`).join("&");
  const url =
    `https://api.trackier.com/v2/publishers/reports` +
    `?start=${dateOnly(start)}&end=${dateOnly(end)}&${kpis}`;
  const resp = await fetch(url, { headers: { "X-Api-Key": apiKey, Accept: "application/json" } });
  if (!resp.ok) {
    throw new Error(`vCommission performance report ${resp.status}: ${(await resp.text()).slice(0, 200)}`);
  }
  const body = await resp.json();
  return { records: Array.isArray(body?.records) ? body.records : [], shape: summariseShape(body) };
}
...
export async function pollConversions({ lookbackDays = 45 } = {}) {
  ...
  for (const net of NETWORKS) {
    ...
    try {
      ...
      if (skipped) {
        // Log the skip. Without a row, the panel keeps showing whatever the
        // LAST logged run was — which after an endpoint failure means a red
        // error line that never goes away even though nothing is failing.
        await logSyncRun({ network: net.logName, status: "skipped", productsSeen: 0, newListings: 0, updatedListings: 0, errorMessage: skipped }).catch(() => {});
  ...
  // Loyalty follows conversions: after every poll, members' ledgers sync
  // to the latest network-confirmed state in one set-based statement.
  try {
    const changed = await syncLoyaltyLedger();
    if (changed) results.push({ loyalty: `${changed} ledger entries created/updated` });
  } catch (err) {
    console.error("Loyalty ledger sync failed:", err.message);
  }
