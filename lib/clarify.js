...
export async function generateClarifyingQuestions(query, locale, country) {
  if (!query || typeof query !== "string" || query.trim().length < 3) return null;
  if (!process.env.ANTHROPIC_API_KEY) return null;

  const language = languageForModel(locale);
  // Same India-or-not split the rest of the pipeline uses for currency
  // (COUNTRY_NAMES/CURRENCIES in app/api/research/route.js) — kept simple
  // here since this is only about which symbol a budget question shows,
  // not the actual price the shopper will see.
  ...
  const system =
    (language === "English"
      ? CLARIFY_PROMPT
      : `${CLARIFY_PROMPT}\n\nWrite the question and the chips in ${language} — the shopper reads ${language}, not English.`) + currencyNote;
