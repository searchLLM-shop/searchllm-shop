...
export function extractQueryTerms(queryText) {
  ...
  for (const w of words) {
    ...
    // 2-letter words are noise except the handful that are real product
    // words (tv). The alias map doubles as that whitelist.
    if (w.length < 3 && !TERM_ALIASES[w]) continue;
    terms.add(w);
...
function augmentQueryForScoring(q) {
  const words = q.replace(/[^a-z0-9\s]/g, " ").split(/\s+/).filter(Boolean);
  const extras = [];
  for (const w of words) for (const alias of TERM_ALIASES[w] || []) extras.push(alias);
  for (let i = 0; i < words.length - 1; i++) {
    const joined = `${words[i]}${words[i + 1]}`;
...
export function extractBudget(queryText) {
  ...
  // "under 75000", "under 50k", "under 1L rs", "below 1.5 lakh" — Indian
  // shoppers write budgets in lakh/k notation as often as in plain digits.
  const m = q.match(/\b(?:under|below|less than|within|upto|up to|max|budget of)\s*(?:rs\.?|inr|\$|£)?\s*(\d+(?:\.\d+)?)\s*(k|l|lakh|lakhs|lac)?\b/);
  if (!m) return null;
  ...
  if (!unit && m[1].length < 2) return null;
...
export function findTopMatchingListings(queryText, approvedListings, userCountry = null, limit = 4) {
  ...
  for (const listing of approvedListings || []) {
    ...
    const titleTokens = `${listing?.product || ""} ${listing?.brand || ""}`
      ...
      .replace(/[^a-z0-9\s]/g, " ")
      .split(/\s+/)
      // Short tokens are noise except real product words (tv) — the alias
      // map doubles as the whitelist, same rule as extractQueryTerms.
      .filter((w) => w.length > 3 || TERM_ALIASES[w]);
