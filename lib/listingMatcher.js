...
import { isBlockedListing, hasAdultContext, mentionsMinors, mentionsAdultAudience } from "@/lib/contentFilter";
...
export function findTopMatchingListings(queryText, approvedListings, userCountry = null, limit = 4, productType = null) {
  ...
  // Children's products are structurally absent from the shortlist
  // whenever the query signals an ADULT audience — either sexual/
  // suggestive context (the original 2026-07-22 fix) or a plain adult-
  // clothing audience word like "women"/"men" (2026-09-18: "women red
  // dress for a party" shortlisted nothing but girls'/baby party dresses;
  // see mentionsAdultAudience's own comment in contentFilter.js for the
  // full story — the catalog holds comparable numbers of both, so this
  // was never a missing-inventory problem). Never applied when the query
  // ITSELF names a minor audience ("dress for my daughter", "girls
  // dress") — that's a real kids' query and kids' products are the right
  // answer to it, exactly as before.
  if (!mentionsMinors(queryText) && (hasAdultContext(queryText) || mentionsAdultAudience(queryText))) {
    approvedListings = approvedListings.filter(
