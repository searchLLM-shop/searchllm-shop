...
import { ENABLE_GERMAN, LOYALTY, REFERRALS } from "@/lib/constants";
import { contentHash, evaluateGate } from "@/lib/publicationGate";
import { encryptPII, decryptPII } from "@/lib/piiCrypto";
import { MINOR_SQL_REGEX } from "@/lib/contentFilter";
...
function getPool() {
  if (!pool) {
    ...
    // SECURITY (2026-08-25 review, corrected same day after this broke
    // production): rejectUnauthorized:false disables TLS certificate
    // verification entirely — the connection is still encrypted, but the
    // app accepts ANY certificate, including one presented by a
    // man-in-the-middle. Turning it on unconditionally broke real
    // production traffic: Supabase's connection pooler presents a
    // certificate that does NOT chain to a CA in Node's default trust
    // store (confirmed live — "self-signed certificate in certificate
    // chain"), unlike a direct-to-RDS connection. The correct fix isn't
    // "verify" vs "don't" — it's supplying the SPECIFIC root certificate
    // Supabase signs with (Dashboard → Project Settings → Database →
    // Connection Info → "Server root certificate"), via DATABASE_CA_CERT,
    // so verification actually has something correct to check against.
    // DATABASE_SSL_INSECURE=1 stays as the fallback for a provider that
    // never gives you a CA cert to pin at all — but pinning the real one
    // is what closes the original gap; the insecure flag on its own does
    // not, it only avoids crashing.
    ...
    if (isLocal) {
      ...
      // Pin the actual issuer — real verification, not a guess about
      // whether Node's built-in CA bundle happens to include it.
...
// to one network. Only ever touches rows that are already pending — never
// re-opens an approved listing. German-only listings are skipped while
// paused: they're hidden from the queue, and a bulk action must never
// change the status of a row the reviewer can't see.
//
// Batched (2026-09-18): a bare single UPDATE with no row cap, run against
// the 3,486,370-row pending Myntra import, would be one enormous
// long-running transaction with a real risk of exceeding this route's
// function timeout — and this route had no maxDuration set at all before
// today, meaning whatever short platform default applied. Batching into
// LIMIT-bounded chunks keeps each individual UPDATE fast regardless of
// total volume. Returns { count, done } — done:false means the time
// budget ran out with pending rows still left; the caller (see
// app/api/admin/listings/route.js) can just call again, since this only
// ever touches rows still in 'pending' — safe to re-invoke any number of
// times, nothing here depends on being called exactly once.
// Defaults tightened 2026-09-18: a 504 hit at these settings on the real
// pending-Myntra run — 45s budget left too little margin under this
// route's 60s maxDuration when an individual batch ran slower than usual
// (real traffic + real DB load, not a fixed cost). Smaller batches and a
// shorter budget trade a few more round trips for a much wider safety
// margin against the hard cutoff.
export async function bulkSetPendingStatus(status, network, { batchSize = 5000, timeBudgetMs = 25000 } = {}) {
      ...
      ? await query(
          `UPDATE listings SET status = $1
           WHERE id IN (
             ...
             WHERE status = 'pending' AND network = $2${GERMAN_ONLY_FILTER}
             LIMIT $3
           )`,
          [status, network, batchSize]
        )
      : await query(
          `UPDATE listings SET status = $1
           WHERE id IN (
             SELECT id FROM listings
             WHERE status = 'pending'${GERMAN_ONLY_FILTER}
             LIMIT $2
           )`,
          [status, batchSize]
        );
    total += result.rowCount;
    if (result.rowCount < batchSize) return { count: total, done: true }; // nothing left to do
    if (Date.now() - startedAt > timeBudgetMs) return { count: total, done: false }; // out of time, more remain
  }
}
  ...
  await query(
    `INSERT INTO microsites
       (title, summary, task_type, learnings, listing_id, query_hash,
        ...
        search_performed, content_hash, clarifications, popularity_level)
    ...
    [
      ...
      "draft",
      // Gate inputs recorded at write time — both are free to compute
      // here, so the publication gate costs zero extra database operations
      // per search. The full gate runs later, at publish time.
      microsite.searchPerformed === true,
      contentHash([microsite.headline, microsite.body, microsite.whoFor, microsite.whoSkip].filter(Boolean).join(' ')),
      // What lib/clarify.js asked and what the shopper answered, if anything
      // — see the clarifications column note in schema.sql.
      JSON.stringify(microsite.clarifications || []),
      // Cosmic-layer popularity gate input — see the popularity_level
      // column note in schema.sql and the popularity check in
      // lib/publicationGate.js.
      microsite.popularityLevel || null,
    ]
  );
}
...
// REMOVED (2026-08-25): getUserPlan/upsertUserPlan read and wrote
// user_plans, which backed the now-removed Plus subscription tier. Nothing
// calls upsertUserPlan any more (the checkout/webhook flow is a single
// flat platform-fee payment now — see payPlatformFeeBlock), so the table
// has been dead since that change; both lookups are removed with it.
...
export async function findCandidateListings(terms, userCountry, limit = 200, excludeMinors = false, rawText = "") {
  if (!Array.isArray(terms) || terms.length === 0) return [];

  // Full-text branch. Word pairs ("whey protein") are excluded here —
  // their words are present individually anyway — and each term is
  // reduced to [a-z0-9] so no user input can reach to_tsquery as syntax.
  // FTS brings English stemming ("powders" finds "powder") and frees
  // matching from depending on the keywords array, which is what lets AI
  // keyword enrichment stay off.
  //
  // QUORUM, not pure OR, once there are enough terms to make pure OR
  // dangerous (2026-09-18, three attempts to get this right):
  //
  // Attempt 1 was capping to the 15 longest terms, on the theory that
  // longer = more specific. Verified via EXPLAIN against the real
  // production catalog — still a sequential scan estimating 1.49M
  // matching rows (39% of the table). The theory was wrong: "designer"
  // and "women" are long words that are ALSO extremely common in a
  // fashion catalog, survived the length cut, and alone (pure OR —
  // matching just ONE term is enough) still guaranteed a huge match
  // count. Length is not a real proxy for rarity.
  //
  // Attempt 2 required at least TWO terms to co-occur — an OR of
  // pairwise ANDs ("t1&t2 | t1&t3 | ..."), a standard technique for
  // taming an over-broad OR-only full-text query. Right shape, wrong
  // pool: picking the pool by length from the full, already-expanded
  // `terms` array mostly selected extractQueryTerms' own SYNTHETIC
  // joined-compound terms ("designerbanarasi", "banarasisaree" —
  // word[i]+word[i+1] concatenated, meant only to catch rare compound-
  // title mismatches like "game pad" finding "Gamepad"). Those are
  // themselves far too rare for requiring two of them together to match
  // almost anything — would have gutted recall instead of fixing
  // selectivity.
  //
  // Attempt 3 (this one): build the quorum pool from the RAW QUERY TEXT
  // directly — a plain tokenization with no compound-joining or half-
  // splitting involved, so every candidate is a genuine word the shopper
  // actually typed. `rawText` is optional and callers should pass the
  // original query string; without it, this falls back to the same
  // (known-imperfect) length-sorted `terms` pool as a last resort rather
  // than failing outright.
  const QUORUM_THRESHOLD = 8; // more terms than this -> switch to quorum
  const QUORUM_POOL = 8; // how many top terms feed the pairwise AND set
  const cleanTerms = terms
    .filter((t) => !t.includes(" "))
    .map((t) => t.replace(/[^a-z0-9]/g, ""))
    .filter((t) => t.length >= 3);
  let ftsQuery;
  if (cleanTerms.length <= QUORUM_THRESHOLD) {
    ...
    const plainWords = (rawText || "")
      ...
      .replace(/[^a-z0-9\s]/g, " ")
      .split(/\s+/)
      .filter((w) => w.length >= 4 && !/^\d+$/.test(w)); // "5000"/"15" alone are noise here, not vocabulary
    const pool = plainWords.length >= 2
      ? Array.from(new Set(plainWords)).sort((a, b) => b.length - a.length).slice(0, QUORUM_POOL)
      : Array.from(new Set(cleanTerms)).sort((a, b) => b.length - a.length).slice(0, QUORUM_POOL);
    const pairs = [];
  ...
  // excludeMinors: applied HERE, at the SQL level, not just in
  // findTopMatchingListings' post-fetch JS filter (lib/listingMatcher.js).
  // Real gap found 2026-09-18: on a "women red dress for a party" query,
  // this catalog's kids'-dress titles rank so densely by search_tsv rank
  // that genuine women's dresses never survived into the top-`limit`
  // candidates at all — the JS-side exclusion had nothing left to rescue.
  // $5 carries the flag as a bind param so this stays one query shape for
  // every caller rather than two SQL strings to keep in sync.
  const { rows } = await query(
    `SELECT id, brand, product, price, category, keywords, network,
...
// DISABLED (2026-08-25, explicit direction): purchase points were removed
// from the platform entirely — see the LOYALTY.POINTS comment in
// lib/constants.js for why (only Awin's conversion feed is actually
// verified; vCommission's and Amazon's are not, so paying out a point on
// an unverifiable claim was the problem, not the solution). No longer
// called from lib/conversions.js's pollConversions(). The early return
// below is the entire change — the query underneath is left intact,
// complete and working (flat points, cap-aware, idempotent re-polls via
// the click_id unique key), so re-enabling this later is deleting one
// line, not re-deriving the design.
  ...
  const { rowCount } = await query(
    `INSERT INTO points_ledger (user_id, click_id, points, status, commission, currency, network, listing_id)
     ...
     WHERE c.conversion_status IS NOT NULL
       ...
       AND (
         up.plan = 'plus'
         OR COALESCE((SELECT SUM(points) FROM points_ledger pl
                      WHERE pl.user_id = c.identity AND pl.status = 'confirmed'), 0) + $1 <= $2
       )
    ...
    // Re-enabling: this referenced LOYALTY.POINTS.PURCHASE, which no
    // longer exists in lib/constants.js — restore that constant too.
    [25, LOYALTY.VOUCHER_UNLOCK_POINTS]
  );
  return rowCount;
}
...
// spent twice while an admin fulfils it. Block-ceiling fields (2026-08-25:
// flat platform-fee model, no plan tier) are computed inline here rather
// than via a second call to getPointsBlockStatus, since the Rewards tab
// needs both in the same round trip anyway.
export async function getRewardsSummary(userId) {
  const [member, balances, ledger, redemptions, blocksPaidRow, searchToday] = await Promise.all([
    query(`SELECT consented_at, kyc_first_name, kyc_last_name, kyc_mobile, kyc_email, kyc_address FROM loyalty_members WHERE user_id = $1`, [userId]),
    query(
      `SELECT
    ...
    query(
      `SELECT COUNT(*) AS blocks_paid FROM user_checkpoints
       WHERE user_id = $1 AND kind = 'platform_fee' AND status = 'paid'`,
      [userId]
    ),
    query(
  ...
  const confirmed = Number(b.confirmed);
  const m = member.rows[0];
  const blocksPaid = Number(blocksPaidRow.rows[0].blocks_paid);
  const ceiling = (blocksPaid + 1) * LOYALTY.POINTS_BLOCK_SIZE;
  return {
    isMember: member.rows.length > 0,
    memberSince: m?.consented_at || null,
    confirmed,
    totalPoints: confirmed,
    pending: Number(b.pending),
    available: Math.max(0, Math.floor(confirmed - Number(b.held_or_redeemed))),
    ledger: ledger.rows,
    // voucher_code decrypted here too (2026-09-14, Qwikcilver UAT test #42:
    // "Card Number and Card PIN received as Spend API response should be
    // encrypted and stored" — this column held plaintext until now, the
    // one KYC-adjacent field the 2026-08-25 encryption pass missed since
    // it isn't technically KYC. Same decryptPII() used everywhere else;
    // legacy plaintext rows pass through unchanged, nothing to backfill.
    redemptions: redemptions.rows.map((r) => ({ ...r, voucher_code: decryptPII(r.voucher_code) })),
    // Block-ceiling status (2026-08-25): earning pauses at `ceiling` until
    // the platform fee for THIS block is paid — see creditSearchPoints/
    // creditClickPoints for the same formula enforced at credit time.
    blocksPaid,
    ceiling,
    atCeiling: confirmed >= ceiling,
    searchPointsToday: Number(searchToday.rows[0].today),
    // RBI KYC on file (see requestRedemption) — prefilled into the
    // redemption confirmation form, always re-shown/re-confirmed there.
    // Name is first/last (2026-08-25); the old single kyc_name column is
    // no longer read here. Columns are encrypted at rest (2026-08-25,
    // security review) — decrypted here, the one place this profile is
    // ever read back for display/prefill.
    storedKyc: m && (m.kyc_first_name || m.kyc_last_name || m.kyc_mobile || m.kyc_email || m.kyc_address)
      ? { firstName: decryptPII(m.kyc_first_name) || "", lastName: decryptPII(m.kyc_last_name) || "", mobile: decryptPII(m.kyc_mobile) || "", email: decryptPII(m.kyc_email) || "", address: decryptPII(m.kyc_address) || "" }
      : null,
  };
}
...
//
// RBI mandate for gift vouchers issued in India (added 2026-08-25, name
// split into first/last the same day): every redemption stores a KYC
// snapshot (first name, last name, mobile, email, address) as it was shown
// and confirmed at that moment — kyc is required, validated by the caller
// (app/api/rewards/route.js) before this is called. The snapshot on the
// redemption row is the audit record of truth; the best-effort upsert into
// loyalty_members below is only the "on file" convenience copy used to
// prefill the confirmation form next time, and its failure must never fail
// the redemption itself.
//
// Encrypted at rest (2026-08-25, security review): every kyc_* value is
// encrypted here, once, before either write — both the redemption snapshot
// and the loyalty_members "on file" copy store ciphertext, never plaintext.
// Encrypting once and reusing the result (rather than calling encryptPII
// per column per query) also means both writes are byte-for-byte
// consistent for the same confirmation.
export async function requestRedemption(userId, points, voucherType, kyc) {
  const encFirstName = encryptPII(kyc.firstName);
  const encLastName = encryptPII(kyc.lastName);
  const encMobile = encryptPII(kyc.mobile);
  const encEmail = encryptPII(kyc.email);
  const encAddress = encryptPII(kyc.address);

  // No plan check (2026-08-25: no plans exist anymore) — just the balance.
  // This is safe on its own because earning itself can never cross an
  // unpaid 250-point block boundary (see creditSearchPoints/
  // creditClickPoints): any balance that exists here was, by construction,
  // already inside a paid block.
  const { rowCount } = await query(
    `INSERT INTO redemptions (user_id, points, voucher_type, kyc_first_name, kyc_last_name, kyc_mobile, kyc_email, kyc_address, kyc_confirmed_at)
     SELECT $1, $2::int, $3, $4, $5, $6, $7, $8, now()
     WHERE (
    ...
    [userId, points, voucherType, encFirstName, encLastName, encMobile, encEmail, encAddress]
  );
  if (rowCount > 0) {
    query(
      `UPDATE loyalty_members
       SET kyc_first_name = $2, kyc_last_name = $3, kyc_mobile = $4, kyc_email = $5, kyc_address = $6, kyc_updated_at = now()
       WHERE user_id = $1`,
      [userId, encFirstName, encLastName, encMobile, encEmail, encAddress]
    ).catch((e) => console.error("KYC profile upsert failed (redemption already recorded):", e.message));
  }
  return rowCount > 0;
}

// The earning ceiling, as a SQL fragment reused by both credit functions
// below: (blocks already paid for + the one free block everyone starts
// with) × block size. Blocks paid = COUNT of 'platform_fee' checkpoints
// with status='paid' (see resolveCheckpoint / payPlatformFeeBlock) — a
// one-way ratchet, never re-locked once paid. A brand-new account with
// zero paid blocks gets a ceiling of exactly POINTS_BLOCK_SIZE (250): the
// first block is always free to EARN into, the fee is only ever required
// to cross OUT of a completed block.
const CEILING_SQL = `(
  COALESCE((SELECT COUNT(*) FROM user_checkpoints
            WHERE user_id = $1 AND kind = 'platform_fee' AND status = 'paid'), 0) + 1
) * $BLOCK_PARAM`;

// Credits search points to a REGISTERED user — every pick earns, up to the
// current block ceiling (2026-08-25: flat platform-fee model, no plan
// involved at all — see the LOYALTY.PLATFORM_FEE_INR comment in
// lib/constants.js). Earning simply pauses at each unpaid 250-point
// boundary for every account, with no exception. One ledger row per
// search (source='search', immediately confirmed — there's no network to
// wait for). The trailing SELECT reads the pre-insert snapshot, so the
// CTE's points are added back explicitly — also correctly reports 0
// earned (not the nominal rate) when the block boundary blocked the insert.
export async function creditSearchPoints(userId) {
  const per = LOYALTY.POINTS.SEARCH;
  const { rows } = await query(
    `WITH ins AS (
       INSERT INTO points_ledger (user_id, points, status, source, note)
       SELECT $1, $2, 'confirmed', 'search', 'search pick'
       WHERE COALESCE((SELECT SUM(points) FROM points_ledger
                       WHERE user_id = $1 AND status = 'confirmed'), 0) + $2
             <= ${CEILING_SQL.replace("$BLOCK_PARAM", "$3")}
       RETURNING points
     )
     SELECT
    ...
    [userId, per, LOYALTY.POINTS_BLOCK_SIZE]
  );
...
// the action closest to revenue earns the most. Still ONE credit per
// product per day (unchanged anti-gaming dedupe): the note carries
// listing+date and the WHERE makes repeat clicks on the same card a no-op,
// otherwise this would be a self-service points mint. Same block-ceiling
// rule as creditSearchPoints, no exceptions. Returns points credited (0 on
// either the dedupe path or the ceiling).
export async function creditClickPoints(userId, listingId) {
  const per = LOYALTY.POINTS.CLICK;
  const { rowCount } = await query(
    `INSERT INTO points_ledger (user_id, points, status, source, note, listing_id)
     ...
     AND COALESCE((SELECT SUM(points) FROM points_ledger
                  WHERE user_id = $1 AND status = 'confirmed'), 0) + $2
         <= ${CEILING_SQL.replace("$BLOCK_PARAM", "$4")}`,
    [userId, per, listingId, LOYALTY.POINTS_BLOCK_SIZE]
  );
  return rowCount > 0 ? per : 0;
}

// --- vCommission purchase points (admin-attested, 2026-09) --------------
//
// Reintroduces purchase points, but sidesteps the exact problem that got
// them removed on 2026-08-25 (see the LOYALTY comment in lib/constants.js):
// instead of trusting vCommission's own never-verified automated
// conversions endpoint, a human admin reads vCommission's periodic
// purchase report and manually confirms each one here. The admin's
// confirmation IS the verification. Scope is deliberately narrow —
// vCommission (Shopsy/Myntra) only, admin-attested only.

// Lists network_clicks rows for the admin panel — vCommission only,
// newest first, optionally scoped to a date range (how the admin actually
// works: cross-referencing vCommission's own report by date). `eligible`
// mirrors the exact join rule the disabled syncLoyaltyLedger() used: the
// click's identity must be a real loyalty member, and the click must have
// happened at/after they joined — a guest click or a click that predates
// opt-in can never earn points, no matter what commission is entered.
export async function getVcommissionClicksForAdmin({ limit = 50, offset = 0, from, to } = {}) {
  const conditions = [`c.network = 'vCommission'`];
  const params = [];
  ...
  const { rows } = await query(
    `SELECT c.id, c.click_id, c.identity, c.created_at, c.commission, c.currency,
            c.listing_id, l.product, l.brand, l.merchant_domain,
            (m.user_id IS NOT NULL AND c.created_at >= m.consented_at) AS eligible,
            pl.points AS credited_points
     FROM network_clicks c
     LEFT JOIN listings l ON l.id = c.listing_id
     LEFT JOIN loyalty_members m ON m.user_id = c.identity
     LEFT JOIN points_ledger pl ON pl.click_id = c.click_id AND pl.source = 'purchase'
     WHERE ${where}
  ...
  const { rows: countRows } = await query(
    `SELECT COUNT(*) AS total FROM network_clicks c WHERE ${where}`,
    params
  );

  return { clicks: rows, total: Number(countRows[0].total) };
}

// The actual credit. Idempotent on click_id (ON CONFLICT DO UPDATE): an
// admin correcting a previously-entered commission updates the same ledger
// row rather than creating a second one. Also records the commission onto
// the network_clicks row itself via updateClickConversion, so the existing
// revenue reporting (getRevenueSummary/getProductPerformance, both read
// network_clicks.commission) reflects vCommission's admin-confirmed
// purchases too, not just the points side.
//
// The `existing` CTE is the subtle correctness detail: when EDITING an
// already-credited row, the ceiling check must subtract what that row
// PREVIOUSLY contributed before re-checking against the new total — the
// same points shouldn't count twice against the ceiling just because a
// correction touched them.
export async function creditVcommissionPurchasePoints({ clickId, commissionInr }) {
  const rate = LOYALTY.VCOMMISSION_PURCHASE_POINTS_RATE;
  const points = Math.round(Number(commissionInr) * rate);

  await updateClickConversion({
    clickId,
    status: "approved",
    orderValue: null,
  ...
  const { rows } = await query(
    `WITH click AS (
       SELECT c.identity, c.listing_id FROM network_clicks c
       JOIN loyalty_members m ON m.user_id = c.identity AND c.created_at >= m.consented_at
       WHERE c.click_id = $1 AND c.network = 'vCommission'
     ),
     existing AS (
       SELECT points FROM points_ledger WHERE click_id = $1
     ),
     upsert AS (
       INSERT INTO points_ledger (user_id, click_id, points, status, commission, currency, network, listing_id, source, note)
       SELECT click.identity, $1, $3, 'confirmed', $2, 'INR', 'vCommission', click.listing_id, 'purchase', 'vCommission purchase, admin-confirmed'
       FROM click
       WHERE (
         COALESCE((SELECT SUM(points) FROM points_ledger WHERE user_id = click.identity AND status = 'confirmed'), 0)
         - COALESCE((SELECT points FROM existing), 0)
         + $3
       ) <= (
         COALESCE((SELECT COUNT(*) FROM user_checkpoints
                   WHERE user_id = click.identity AND kind = 'platform_fee' AND status = 'paid'), 0) + 1
       ) * $4
       ON CONFLICT (click_id) DO UPDATE SET
         points = EXCLUDED.points, commission = EXCLUDED.commission, currency = EXCLUDED.currency,
         listing_id = EXCLUDED.listing_id, updated_at = now()
       RETURNING points, user_id
     )
     SELECT points, user_id FROM upsert`,
    [clickId, commissionInr, points, LOYALTY.POINTS_BLOCK_SIZE]
  );

  if (!rows.length) {
    return {
      ok: false,
      reason: "Not eligible — this click isn't from a registered rewards member (or predates their opt-in), or crediting would exceed their current points-block ceiling.",
    };
  }
  return { ok: true, points: Number(rows[0].points), userId: rows[0].user_id };
}

// --- Referral programme (2026-09-12) -------------------------------------
//
// A registered user shares their own link via their own WhatsApp — we
// never send anything ourselves, never see the friend's phone number (see
// app/api/referrals/route.js). Each CONFIRMED new registration earns the
// referrer REFERRALS.POINTS_PER_REFERRAL points, capped at
// REFERRALS.MAX_REFERRALS for life.

// Lazily generates a short, permanent referral code on first need — never
// regenerated once created, same "create once" precedent as
// lib/guestId.js's cookie. Collision retry is cheap insurance at 8 hex
// chars (32 bits), not a realistic concern at this scale.
export async function getOrCreateReferralCode(userId) {
  ...
  for (let i = 0; i < 5; i++) {
    const code = newToken(4);
    try {
      await query(`INSERT INTO referral_codes (user_id, code) VALUES ($1, $2)`, [userId, code]);
      return code;
    } catch (err) {
      if (err.code === "23505") continue; // code collision — retry with a fresh one
      throw err;
    }
  }
...
// Cheap existence check for the /r/[code] landing route — no need to
// resolve the actual referrer here, that only matters at confirm time.
export async function referralCodeExists(code) {
  if (!code) return false;
  const { rows } = await query(`SELECT 1 FROM referral_codes WHERE code = $1`, [code]);
  return rows.length > 0;
}

// Everything the Referrals tab shows: the share link's code, how many
// successful referrals so far (and whether the cap is reached), total
// points earned from referrals specifically, and a lightweight history —
// dates only, the referred person's identity is never surfaced to the
// referrer.
export async function getReferralsSummary(userId) {
  const code = await getOrCreateReferralCode(userId);
  const { rows } = await query(
    `SELECT points, created_at FROM referrals WHERE referrer_user_id = $1 ORDER BY created_at DESC`,
    [userId]
  );
  const referralsMade = rows.length;
  const pointsEarned = rows.reduce((sum, r) => sum + Number(r.points), 0);
  return {
    code,
    referralsMade,
    maxReferrals: REFERRALS.MAX_REFERRALS,
    capped: referralsMade >= REFERRALS.MAX_REFERRALS,
    pointsEarned,
    pointsPerReferral: REFERRALS.POINTS_PER_REFERRAL,
    history: rows.map((r) => ({ points: Number(r.points), createdAt: r.created_at })),
  };
}

// The actual credit. Idempotent on referred_user_id (a UNIQUE constraint on
// `referrals`) — a person can complete exactly one successful referral,
// ever, no matter how many times a confirm attempt fires (reloads,
// retries). Self-referral and the per-referrer cap are both enforced in
// the same atomic statement as the insert, not as separate pre-checks, so
// there's no race between checking and inserting.
export async function confirmReferral({ referredUserId, code }) {
  ...
  const { rows } = await query(
    `WITH ref AS (
     ...
     capcheck AS (
       SELECT COUNT(*) AS n FROM referrals WHERE referrer_user_id = (SELECT user_id FROM ref)
     ),
     ins AS (
       INSERT INTO referrals (referrer_user_id, referred_user_id, code, points)
       SELECT ref.user_id, $1, $2, $3
       FROM ref
       WHERE ref.user_id IS NOT NULL
         AND ref.user_id != $1
         AND (SELECT n FROM capcheck) < $4
       ON CONFLICT (referred_user_id) DO NOTHING
       RETURNING referrer_user_id
     )
     SELECT referrer_user_id FROM ins`,
    [referredUserId, code, points, REFERRALS.MAX_REFERRALS]
  );
  if (!rows.length) return { ok: false };

  const referrerUserId = rows[0].referrer_user_id;
  // Same block-ceiling gate as creditSearchPoints/creditVcommissionPurchasePoints
  // — referral points are just another confirmed source, no exception.
  await query(
    `INSERT INTO points_ledger (user_id, points, status, source, note)
     SELECT $1, $2, 'confirmed', 'referral', 'referral:' || $3
     WHERE COALESCE((SELECT SUM(points) FROM points_ledger WHERE user_id = $1 AND status = 'confirmed'), 0) + $2
           <= ${CEILING_SQL.replace("$BLOCK_PARAM", "$4")}`,
    [referrerUserId, points, referredUserId, LOYALTY.POINTS_BLOCK_SIZE]
  );
  // Best-effort — a referrer's points must be visible/redeemable even if
  // they never separately clicked "Join"; failure here must never undo
  // the referral credit above.
  try { await joinLoyalty(referrerUserId); } catch (err) { console.error("joinLoyalty on referral failed:", err.message); }

  return { ok: true, referrerUserId, points };
}

// Admin report: platform-wide referral activity.
export async function getReferralsReport() {
  const [totals, topReferrers] = await Promise.all([
    ...
    query(
      `SELECT referrer_user_id, COUNT(*) AS referrals, SUM(points) AS points, MAX(created_at) AS last_referral_at
       FROM referrals
  ...
  return {
    totalReferrals: Number(totals.rows[0].total_referrals),
    totalPoints: Number(totals.rows[0].total_points),
    uniqueReferrers: Number(totals.rows[0].unique_referrers),
...
// The status the Rewards UI and the homepage banner need: current
// confirmed total, the ceiling it's up against, whether earning is
// currently paused at that ceiling, and how many blocks have been paid
// for in total (informational).
export async function getPointsBlockStatus(userId) {
  const { rows } = await query(
    `SELECT
       COALESCE((SELECT SUM(points) FROM points_ledger WHERE user_id = $1 AND status = 'confirmed'), 0) AS confirmed,
       COALESCE((SELECT COUNT(*) FROM user_checkpoints
                 WHERE user_id = $1 AND kind = 'platform_fee' AND status = 'paid'), 0) AS blocks_paid`,
    [userId]
  );
  const confirmed = Number(rows[0].confirmed);
  const blocksPaid = Number(rows[0].blocks_paid);
  const ceiling = (blocksPaid + 1) * LOYALTY.POINTS_BLOCK_SIZE;
  return {
    confirmed,
    blocksPaid,
    ceiling,
    atCeiling: confirmed >= ceiling,
    nextBlockCeiling: ceiling + LOYALTY.POINTS_BLOCK_SIZE,
  };
}

// Records a successful ₹PLATFORM_FEE_INR payment — called from the
// Razorpay webhook on payment_link.paid for a "platform_fee" note. Reuses
// resolveCheckpoint's existing (user, kind, block_number) shape (kind=
// 'platform_fee' rather than the old 'search') — block_number auto-
// increments per user, which is exactly "how many blocks has this account
// paid for", so getPointsBlockStatus's COUNT above needs no separate
// counter column. Idempotent the same way resolveCheckpoint always was:
// a webhook retry just fails the UNIQUE(user_id, kind, block_number)
// constraint on an already-recorded block and no-ops.
export async function payPlatformFeeBlock(userId) {
  return resolveCheckpoint(userId, "platform_fee", "paid");
}

// Guest points are VIRTUAL — computed live from today's activity under the
// guest cookie identity, never stored, gone at midnight by construction.
// Search and click only (2026-08-25: purchase points removed from the
// platform entirely, see the LOYALTY.POINTS comment in lib/constants.js —
// this was also the ONE place a guest's purchase activity was ever looked
// at, so removing it means guest point calculation no longer touches
// conversion data at all).
export async function getGuestDaySummary(guestIdentity) {
  const { rows } = await query(
    `SELECT
       COALESCE((SELECT SUM(search_count) FROM usage_daily
                 WHERE identity = $1 AND day = (now() AT TIME ZONE 'utc')::date), 0) AS searches,
       COALESCE((SELECT COUNT(DISTINCT listing_id) FROM network_clicks
                 WHERE identity = $1 AND listing_id IS NOT NULL
                   AND created_at::date = (now() AT TIME ZONE 'utc')::date), 0) AS clicks`,
    [guestIdentity]
  );
  const r = rows[0];
  const searches = Number(r.searches);
  const clicks = Number(r.clicks);
  const total = searches * LOYALTY.POINTS.GUEST_PER_PICK + clicks * LOYALTY.POINTS.CLICK;
  return { searches, clicks, total };
}

// Convenience wrapper for the common case (most callers just want today's
// total, e.g. the header chip and the post-search banner).
export async function getGuestDayPoints(guestIdentity) {
  return (await getGuestDaySummary(guestIdentity)).total;
}

// The registration hook: a newly signed-in user carries their old guest
// cookie, so TODAY's guest points (search and click) convert into real
// ledger points, once, per source. The note carries the date, and the
// WHERE makes each claim idempotent — re-calls the same day insert nothing
// more.
export async function claimGuestDayPoints(userId, guestIdentity) {
  if (!guestIdentity) return 0;
  const summary = await getGuestDaySummary(guestIdentity);
  if (summary.total <= 0) return 0;

  const claimOne = async (source, points) => {
    if (points <= 0) return 0;
    const { rowCount } = await query(
      `INSERT INTO points_ledger (user_id, points, status, source, note)
       SELECT $1, $2, 'confirmed', $3, 'guest day claim ' || (now() AT TIME ZONE 'utc')::date
       WHERE NOT EXISTS (
         SELECT 1 FROM points_ledger
         WHERE user_id = $1 AND source = $3
  ...
  const claimed = await Promise.all([
    claimOne("search", summary.searches * LOYALTY.POINTS.GUEST_PER_PICK),
    claimOne("click", summary.clicks * LOYALTY.POINTS.CLICK),
  ]);
  return claimed.reduce((a, b) => a + b, 0);
}
...
export async function getRewardsReport() {
  const [issuance, liability, byVoucher, feePayments, atCeiling, dailyIssuance] = await Promise.all([
    query(
    ...
    // Platform fee payments (2026-08-25: flat model, no plan tier) — every
    // paid block is one 'platform_fee' checkpoint, so a straight count
    // times the fee is exactly the real revenue this mechanism has
    // generated, reconcilable against Razorpay's own records.
    query(
      `SELECT COUNT(*) AS blocks_paid
       FROM user_checkpoints WHERE kind = 'platform_fee' AND status = 'paid'`
    ),
    // Members currently AT their ceiling — earning paused, a platform-fee
    // payment away from their next voucher. This is who the "you've
    // reached 250 points" banner is showing to right now; high and
    // growing without matching payment volume is the signal the fee or
    // the framing needs a look.
    query(
      `SELECT COUNT(*) AS members FROM (
         SELECT pl.user_id
         FROM points_ledger pl
         JOIN loyalty_members m ON m.user_id = pl.user_id
         GROUP BY pl.user_id
         HAVING COALESCE(SUM(pl.points) FILTER (WHERE pl.status = 'confirmed'), 0) >=
                (COALESCE((SELECT COUNT(*) FROM user_checkpoints uc
                           WHERE uc.user_id = pl.user_id AND uc.kind = 'platform_fee' AND uc.status = 'paid'), 0) + 1) * $1
       ) at_ceiling`,
      [LOYALTY.POINTS_BLOCK_SIZE]
    ),
    // Daily issuance trend, last 30 days — same shape as the platform-wide
    // daily trend elsewhere in reporting, scoped to points.
    query(
      `SELECT created_at::date AS day, source, COALESCE(SUM(points), 0) AS points, COUNT(*) AS entries
       FROM points_ledger
  ...
  return {
    ...
    platformFeeBlocksPaid: Number(feePayments.rows[0]?.blocks_paid || 0),
    platformFeeRevenue: Number(feePayments.rows[0]?.blocks_paid || 0) * LOYALTY.PLATFORM_FEE_INR,
    membersAtCeiling: Number(atCeiling.rows[0]?.members || 0),
    dailyIssuance: dailyIssuance.rows,
    byVoucher: byVoucher.rows,
    feedback: feedback.rows,
  };
}

// REMOVED (2026-08-25): getLifecycleStatus implemented the Plus-only
// query-cycle gate (25 queries with no purchase → Increase Usage). Both
// the plan tier and the Increase Usage mechanism it protected are gone —
// see LOYALTY.PLATFORM_FEE_INR in lib/constants.js. Research access is no
// longer gated at the query level for anyone, ever; the only remaining
// lever in the whole rewards system is whether NEW POINTS keep
// accumulating (creditSearchPoints/creditClickPoints's block ceiling),
// which never touches search access itself. Callers updated: the click
// gate in app/out/[listingId]/route.js and the search gate in
// app/api/research/route.js both no longer call this.

// The one thing worth carrying over from the old lifecycle engine: IP-level
// fair-use limits still exempt anyone with a real payment or purchase
// history (shared carrier IPs must never punish a paying customer). Used
// by both the search gate (app/api/research/route.js) and the click gate
// (app/out/[listingId]/route.js).
export async function hasPaymentCredit(identity) {
  const { rows } = await query(
    `SELECT
       (EXISTS(SELECT 1 FROM user_checkpoints WHERE user_id = $1 AND kind = 'platform_fee' AND status = 'paid')
        OR EXISTS(SELECT 1 FROM network_clicks WHERE identity = $1 AND conversion_status = 'approved')) AS has_credit`,
    [identity]
  );
  return rows[0]?.has_credit === true;
}
...
// Everything the header (and now the homepage rewards banner) needs in ONE
// query: plan, today's picks, the points balance, and the raw confirmed
// total (added 2026-08-25 — distinct from "balance", which subtracts
// redemptions; the homepage nudge needs the raw total against the 250-point
// ceiling, not what's left to spend). Guest day-points need no separate
// query at all — they're computed live, see getGuestDayPoints.
export async function getHeaderSnapshot(identity, userId) {
  const { rows } = await query(
    `SELECT
       ...
       COALESCE((SELECT SUM(points) FROM points_ledger
                 WHERE user_id = $2 AND status = 'confirmed'), 0) AS confirmed,
       COALESCE((SELECT SUM(points) FROM points_ledger
       ...
       COALESCE((SELECT COUNT(*) FROM user_checkpoints
                 WHERE user_id = $2 AND kind = 'platform_fee' AND status = 'paid'), 0) AS blocks_paid`,
    [identity, userId || null]
  );
  const r = rows[0];
  const confirmed = Math.floor(Number(r.confirmed));
  const blocksPaid = Number(r.blocks_paid);
  const ceiling = (blocksPaid + 1) * LOYALTY.POINTS_BLOCK_SIZE;
  return {
    used: Number(r.used),
    totalPoints: confirmed,
...
// SECURITY (2026-08-25 review): a hardcoded salt makes this claim false —
// IPv4 is only ~4.3 billion addresses, and SHA-256 is fast enough that
// anyone with DB read access and knowledge of a PUBLIC, in-source salt can
// precompute a full rainbow table and reverse every ip_hash in minutes,
// which is exactly what the Privacy Policy (§6B) promises can't happen.
// IP_HASH_SALT MUST be set in Vercel (a long random value, e.g.
// `openssl rand -hex 32`) for that promise to actually hold — the
// hardcoded fallback below only exists so a missing env var degrades to
// today's (weak) behavior instead of crashing every search request.
const IP_SALT = process.env.IP_HASH_SALT || "sllm-ip-fairuse-v1";
...
// KYC fields included (added 2026-08-25, name split into first/last the
// same day) — the admin needs the confirmed name/mobile/email/address to
// actually address and issue the voucher under the RBI mandate; the
// redemption row is the audit snapshot of what the member confirmed at
// that moment (see requestRedemption). Columns are encrypted at rest —
// decrypted here, the only place this queue is ever read.
...
// Admin: the FULL redemption history for download (added 2026-08-25) — no
// row cap, unlike getRedemptionQueue above (which exists to drive the
// day-to-day fulfilment workflow, deliberately bounded to the most recent
// 100). `from`/`to` are optional ISO dates (inclusive); omitted entirely
// means "everything, no date bound at all" — a compliance/audit export's
// natural default is "all of it", unlike the rest of the reporting suite
// (getReportSummary) which defaults to a 30-day window for a dashboard
// overview. Same decrypt-on-read posture as every other KYC read path.
export async function getAllRedemptions({ from = null, to = null } = {}) {
  const { rows } = await query(
    `SELECT id, user_id, points, voucher_type, status, voucher_code, created_at, fulfilled_at,
            kyc_first_name, kyc_last_name, kyc_mobile, kyc_email, kyc_address, kyc_confirmed_at
     FROM redemptions
...
export async function getClicksReport(days = 30, range = null) {
  ...
  const [byDay, altDemand, queriesByDay] = await Promise.all([
    ...
    query(
      `SELECT created_at::date AS day, query AS query_text, matched
       FROM search_queries
       WHERE created_at::date BETWEEN $1::date AND $2::date
       ORDER BY created_at DESC
       LIMIT 5000`,
...
export async function getReportSummary(days = 30, range = null) {
  const { from, to } = normalizeRange(days, range);
  const [totals, daily, topProducts, byNetwork, topUsers, activity, inventory, sources, campaigns, revenue, inventoryByCountry, inventoryByCategory, rewardsReport, clicksReport, pwaStats, publicationStats] =
    await Promise.all([
      query(
        `SELECT
           (SELECT COUNT(DISTINCT identity) FROM events WHERE event_type = 'visit') AS total_visitors,
           -- 2026-08-25: was COUNT(*) FROM user_plans, which only ever held
           -- a row for someone who'd gone through the (now-removed) Plus
           -- payment flow — it would report 0 forever going forward.
           -- points_ledger.user_id is populated for every signed-in search
           -- regardless of rewards membership, so DISTINCT user_id there is
           -- the accurate count of registered accounts who've searched.
           (SELECT COUNT(DISTINCT user_id) FROM points_ledger) AS registered_users,
      ...
      getPublicationStats().then((r) => ({ rows: [r] })),
    ]);
  ...
  return {
    ...
    pwa: pwaStats.rows[0],
    publication: publicationStats.rows[0],
  };
}
...
// Publishing is now GATED (2026-07-29). Previously this published every
// draft in one statement — the scaled-publishing pattern that gets domains
// sitewide-demoted when a large share of URLs read as unhelpful. Now each
// draft is evaluated, only passers are published, and gate_result is
// written for every draft either way so the thresholds can be tuned
// against real decisions rather than guesses.
//
// Set force=true to publish a specific draft despite the gate (human
// override for a page you've read and judged worth it).
export async function bulkPublishDrafts({ force = false } = {}) {
  const [drafts, published] = await Promise.all([
    query(
      `SELECT id, slug, task_type, headline, body, who_for, who_skip, alternatives, search_performed, popularity_level
       FROM microsites WHERE status = 'draft' AND slug IS NOT NULL LIMIT 500`
    ),
    query(
      `SELECT id, slug, content_hash,
              COALESCE(headline,'') || ' ' || COALESCE(body,'') || ' ' ||
              COALESCE(who_for,'') || ' ' || COALESCE(who_skip,'') AS text
       FROM microsites WHERE status = 'published' LIMIT 2000`
    ),
  ]);

  const pool = published.rows.map((p) => ({
    id: p.id, slug: p.slug, text: p.text, contentHash: p.content_hash,
  }));
    ...
    const result = evaluateGate(
      {
        ...
        searchPerformed: row.search_performed === true,
        popularityLevel: row.popularity_level || null,
      },
      pool
    );
    await query(
      `UPDATE microsites SET gate_result = $2, content_hash = $3 WHERE id = $1`,
      [row.id, JSON.stringify(result), result.contentHash]
    );
    if (result.passed || force) {
      toPublish.push(row.id);
      // A newly published page joins the novelty pool immediately, so a
      // batch can't publish two near-identical drafts in the same run.
      pool.push({
        id: row.id, slug: row.slug, contentHash: result.contentHash,
        text: [row.headline, row.body, row.who_for, row.who_skip].filter(Boolean).join(" "),
      });
    } else {
      for (const reason of result.failedOn) failures[reason] = (failures[reason] || 0) + 1;
    }
  }
  ...
  if (toPublish.length) {
    ...
    publishedCount = rowCount;
  }
  return {
    published: publishedCount,
    evaluated: drafts.rows.length,
    blocked: drafts.rows.length - toPublish.length,
    failures,
  };
}

// Publication-health metrics for the Reports tab. Publication rate climbing
// above ~15% means the gate has gone soft.
export async function getPublicationStats() {
  ...
  const failureRows = await query(
    `SELECT f AS reason, COUNT(*) AS count
     FROM microsites, LATERAL jsonb_array_elements_text(COALESCE(gate_result->'failedOn','[]'::jsonb)) f
     WHERE status = 'draft'
  ...
  return {
    ...
    total,
    publicationRate: total > 0 ? Math.round((Number(r.published) / total) * 1000) / 10 : 0,
    failures: failureRows.rows.map((x) => ({ reason: x.reason, count: Number(x.count) })),
  };
}
