...
export async function findCandidateListings(terms, userCountry, limit = 200, excludeMinors = false, rawText = "") {
  ...
  //
  // TWO-STAGE query (2026-09-18, after three failed attempts at taming
  // the tsquery itself — capping to the longest terms, then a quorum of
  // co-occurring terms drawn two different ways — see git history on this
  // function for the full story of why each one still left a sequential
  // scan over a seven-figure "matching" set: this catalog's fashion
  // titles are formulaic enough that even requiring 2 real words like
  // "women"+"saree" to co-occur barely narrowed anything, because most
  // banarasi-saree listings contain BOTH as routine boilerplate). None of
  // those approaches address the actual cost driver: ts_rank has to be
  // computed and sorted for every row the WHERE clause matches BEFORE the
  // outer LIMIT can apply, and for a sufficiently generic query on a
  // 3.8M-row catalog, "every row the WHERE clause matches" can genuinely
  // be over a million rows — trying to shrink the match count away
  // (rather than bounding the WORK) keeps failing because the vocabulary
  // itself is a red herring: it's not that these terms are unusually
  // broad, it's that the catalog just got dramatically bigger.
  //
  // The actual fix: an inner CTE with a LIMIT and NO ORDER BY, so
  // Postgres never has to compute ts_rank (or examine the full matching
  // set) at all — it can stop as soon as it's gathered enough rows,
  // regardless of how many rows would have matched in total. The outer
  // query then ranks only that bounded seed set, which is cheap
  // regardless. Trade-off: the seed set isn't guaranteed to contain the
  // GLOBALLY top-ranked matches if the true matching set exceeds
  // SEED_LIMIT — acceptable here because findCandidateListings was never
  // the precision layer (findTopMatchingListings' JS scorer is, with its
  // own multi-term weighting, product-type boost, and quality nudges);
  // this only ever needed to hand it a reasonable pool, not a perfect one.
  const SEED_LIMIT = 5000;
  const { rows } = await query(
    `WITH seed AS (
       SELECT id, brand, product, price, category, keywords, network,
              network_link, pitch, regions, image_url, merchant_domain, discount,
              rating, rating_count, search_tsv
       FROM listings
     ...
     LIMIT $3`,
    [terms, userCountry, limit, ftsQuery, excludeMinors, MINOR_SQL_REGEX, SEED_LIMIT]
  );
  return rows;
}
