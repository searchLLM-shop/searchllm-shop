...
export async function getSearchQueryStats(days = 30) {
...
export async function getRecentSearchQueries(days = 30, page = 1, pageSize = 50) {
...
export async function getPerformanceByCategory(days = 30) {
  const { rows } = await query(
    `SELECT COALESCE(l.category, 'other') AS category,
     ...
     WHERE c.created_at >= (now() AT TIME ZONE 'utc')::date - ($1::int - 1)
     GROUP BY 1
...
export async function getProductPerformance(days = 30, page = 1, pageSize = 50) {
  const offset = (Math.max(1, page) - 1) * pageSize;
  const [items, count] = await Promise.all([
    query(
...
export async function getReportSummary(days = 30) {
  const [totals, daily, topProducts, byNetwork, topUsers, activity, plans, inventory, sources, campaigns, revenue, inventoryByCountry, inventoryByCategory] =
    await Promise.all([
      query(
        `SELECT
      ...
      getRevenueSummary(days).then((r) => ({ rows: [r] })),
