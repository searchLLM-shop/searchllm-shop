  ...
  const [member, balances, ledger, redemptions, plan, searchToday] = await Promise.all([
    ...
    query(
      `SELECT COALESCE(SUM(points), 0) AS today FROM points_ledger
       WHERE user_id = $1 AND source = 'search' AND created_at::date = (now() AT TIME ZONE 'utc')::date`,
      [userId]
    ),
  ]);
...
// Credits search points to a REGISTERED user, respecting the daily cap.
// One ledger row per search (source='search', immediately confirmed —
// there's no network to wait for). Returns points actually earned this
// call (0 when capped), plus today's running total for the UI message.
export async function creditSearchPoints(userId) {
  const per = LOYALTY.SEARCH_POINTS.USER_PER_PICK;
  const cap = LOYALTY.SEARCH_POINTS.USER_DAILY_CAP;
  // Insert-if-under-cap in one statement — same race-safety idea as
  // redemptions: the cap check and the insert can't see different states.
  const { rowCount } = await query(
  ...
  return { earned: rowCount > 0 ? per : 0, todayTotal: Number(rows[0].today), capped: rowCount === 0 };
}
