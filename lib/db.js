...
export async function creditSearchPoints(userId) {
  ...
  // Lifetime engagement cap (picks + clicks): the check and the insert are
  // one statement, so concurrent searches can't both slip under the cap.
  await query(
  ...
  return { earned: per, todayTotal: Number(rows[0].today), capped: false };
}
...
// The lifecycle engine, anchor-based: every confirmed purchase and every
// paid recharge moves the ANCHOR; counters measure activity since it, and
// the checks recur for life. BOTH gates block (decision 2026-07-23): picks
// stop at the plan's search limit, affiliate clicks stop at the plan's
// click limit, until a purchase or Increase Usage resets the cycle. Plus
// gets higher allowances (GATE_LIMITS.plus), not unlimited. hasCredit
// (ever purchased or paid) also exempts a user from the IP-level gate.
export async function getLifecycleStatus(userId) {
  const [anchorRow, plan, credit] = await Promise.all([
    query(
      `SELECT GREATEST(
    ...
    query(`SELECT plan FROM user_plans WHERE user_id = $1`, [userId]),
    query(
      `SELECT EXISTS(SELECT 1 FROM user_checkpoints WHERE user_id = $1 AND status = 'paid')
  ...
  const limits = LOYALTY.GATE_LIMITS[plan.rows[0]?.plan === "plus" ? "plus" : "free"];

  const { rows } = await query(
    `SELECT
       COALESCE((SELECT SUM(search_count) FROM usage_daily
                 WHERE identity = $1 AND day >= $2::date), 0) AS searches_since,
       (SELECT COUNT(*) FROM network_clicks
        WHERE identity = $1 AND created_at > $2) AS clicks_since`,
    [userId, anchor]
  );
  const searchesSince = Number(rows[0].searches_since);
  const clicksSince = Number(rows[0].clicks_since);

  return {
    searches: searchesSince,
    clicks: clicksSince,
    limits,
    searchGated: searchesSince >= limits.searches,
    clickGated: clicksSince >= limits.clicks,
    hasCredit: credit.rows[0].has_credit === true,
  };
}
...
export async function getIpGate(ipHash) {
...
export async function getReportSummary(days = 30, range = null) {
  const { from, to } = normalizeRange(days, range);
  const [totals, daily, topProducts, byNetwork, topUsers, activity, plans, inventory, sources, campaigns, revenue, inventoryByCountry, inventoryByCategory, rewardsReport] =
    await Promise.all([
      query(
        `SELECT
