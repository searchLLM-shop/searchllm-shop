...
function getPool() {
  if (!pool) {
    ...
    // SECURITY (2026-08-25 review): rejectUnauthorized:false previously
    // disabled TLS certificate verification entirely — the connection was
    // still encrypted, but the app would accept ANY certificate, including
    // one presented by a man-in-the-middle, without complaint. Supabase
    // (and Vercel Postgres, Neon) all present certificates chained to a
    // publicly-trusted CA, so proper verification should just work; this
    // defaults to secure now, with DATABASE_SSL_INSECURE=1 as an explicit,
    // loud escape hatch if a specific provider genuinely needs it — that
    // failure mode (connection refused, visible in logs) is far preferable
    // to silently accepting any certificate on every connection.
    const sslInsecure = process.env.DATABASE_SSL_INSECURE === "1";
    pool = new Pool({
      connectionString,
      ssl: isLocal ? false : { rejectUnauthorized: !sslInsecure },
