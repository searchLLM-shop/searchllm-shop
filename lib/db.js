...
export async function getRewardsSummary(userId) {
  ...
  return {
    ...
    storedKyc: m && (m.kyc_first_name || m.kyc_last_name || m.kyc_mobile || m.kyc_email || m.kyc_address)
      ? { firstName: m.kyc_first_name || "", lastName: m.kyc_last_name || "", mobile: m.kyc_mobile || "", email: m.kyc_email || "", address: m.kyc_address || "" }
      : null,
  };
}
...
export async function requestRedemption(userId, points, voucherType, kyc) {
  ...
  const { rowCount } = await query(
    ...
    [userId, points, voucherType, kyc.firstName, kyc.lastName, kyc.mobile, kyc.email, kyc.address]
  );
  if (rowCount > 0) {
    query(
      `UPDATE loyalty_members
       ...
       WHERE user_id = $1`,
      [userId, kyc.firstName, kyc.lastName, kyc.mobile, kyc.email, kyc.address]
...
// that moment (see requestRedemption).
