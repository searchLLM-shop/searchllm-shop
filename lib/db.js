...
export async function getLifecycleStatus(identity) {
  ...
  const { rows } = await query(
    `WITH anchor AS (
     ...
     SELECT
       ...
       COALESCE((SELECT SUM(points) FROM points_ledger
                 WHERE user_id = $1 AND source IN ('search', 'click')), 0) AS engagement_points
     FROM anchor a`,
    [identity]
  );
  const r = rows[0];
  ...
  return {
    ...
    engagementPoints: Number(r.engagement_points),
...
export async function getHeaderSnapshot(identity, userId) {
  const { rows } = await query(
    `SELECT
       ...
       COALESCE((SELECT SUM(points) FROM points_ledger
                 WHERE user_id = $2 AND source IN ('search', 'click')), 0) AS engagement`,
  ...
  return {
    ...
    engagementPoints: Number(r.engagement),
  };
}
