...
// confirmed / reversed. Points are now FLAT (LOYALTY.POINTS.PURCHASE,
// redesigned 2026-08-25) — identical regardless of order size or plan; the
...
export async function getRewardsSummary(userId) {
  ...
  return {
    ...
    // uncapped) — totalPoints is the public name for what "confirmed"
    // already was structurally. Reaching VOUCHER_UNLOCK_POINTS is a nudge
    // to upgrade and claim a voucher, not a forced gate.
...
// Credits search points to a REGISTERED user — every pick earns, UNCAPPED
// (redesigned 2026-08-25: the old lifetime engagement cap is gone, points
// accumulate without limit). One ledger row per search (source='search',
// immediately confirmed — there's no network to wait for).
export async function creditSearchPoints(userId) {
  ...
  const { rows } = await query(
    `WITH ins AS (
       ...
       VALUES ($1, $2, 'confirmed', 'search', 'search pick')
       RETURNING points
     )
     SELECT
  ...
  return { earned, todayTotal: Number(rows[0].today_total) };
}
...
// otherwise this would be a self-service points mint. Earning itself is
// UNCAPPED (redesigned 2026-08-25). Returns points credited (0 on the
// dedupe path).
