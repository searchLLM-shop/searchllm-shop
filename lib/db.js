...
export async function getRewardsSummary(userId) {
  const [member, balances, ledger, redemptions, plan, searchToday] = await Promise.all([
    query(`SELECT consented_at, kyc_name, kyc_mobile, kyc_email, kyc_address FROM loyalty_members WHERE user_id = $1`, [userId]),
    query(
      `SELECT
  ...
  return {
    ...
    storedKyc: m && (m.kyc_name || m.kyc_mobile || m.kyc_email || m.kyc_address)
      ? { name: m.kyc_name || "", mobile: m.kyc_mobile || "", email: m.kyc_email || "", address: m.kyc_address || "" }
      : null,
  };
}
...
//
// RBI mandate for gift vouchers issued in India (added 2026-08-25): every
// redemption stores a KYC snapshot (name/mobile/email/address) as it was
// shown and confirmed at that moment — kyc is required, validated by the
// caller (app/api/rewards/route.js) before this is called. The snapshot on
// the redemption row is the audit record of truth; the best-effort upsert
// into loyalty_members below is only the "on file" convenience copy used to
...
export async function requestRedemption(userId, points, voucherType, kyc) {
  ...
  const { rowCount } = await query(
    `INSERT INTO redemptions (user_id, points, voucher_type, kyc_name, kyc_mobile, kyc_email, kyc_address, kyc_confirmed_at)
     SELECT $1, $2::int, $3, $4, $5, $6, $7, now()
    ...
    [userId, points, voucherType, kyc.name, kyc.mobile, kyc.email, kyc.address]
  );
  if (rowCount > 0) {
    query(
      `UPDATE loyalty_members
       SET kyc_name = $2, kyc_mobile = $3, kyc_email = $4, kyc_address = $5, kyc_updated_at = now()
       WHERE user_id = $1`,
      [userId, kyc.name, kyc.mobile, kyc.email, kyc.address]
...
// KYC fields included (added 2026-08-25) — the admin needs the confirmed
// name/mobile/email/address to actually address and issue the voucher
// under the RBI mandate; the redemption row is the audit snapshot of what
// the member confirmed at that moment (see requestRedemption).
export async function getRedemptionQueue() {
  const { rows } = await query(
    `SELECT id, user_id, points, voucher_type, status, voucher_code, created_at, fulfilled_at,
            kyc_name, kyc_mobile, kyc_email, kyc_address, kyc_confirmed_at
     FROM redemptions
