...
export async function findCandidateListings(terms, userCountry, limit = 200, excludeMinors = false) {
  ...
  //
  // Capped at the 15 LONGEST terms (2026-09-18): a long, attribute-heavy
  // query — refinement chips chained on top of each other easily produce
  // 25-30 extracted terms — OR's every one into this single tsquery.
  // That's fine for recall at small scale, but caught live right after
  // the Myntra bulk-approve grew the approved pool 22x (169K -> 3.8M): a
  // 29-term OR query matched an estimated 1.76M rows (46% of the table),
  // broad enough that the planner correctly abandons the GIN index for a
  // sequential scan — a rational plan for that selectivity, just an
  // expensive one at this volume. Longer terms are more specific ("silk"
  // is common, "banarasi" isn't), so capping to the longest ones trades a
  // little recall for a lot of selectivity — the genuinely relevant
  // listings still match on their most distinctive words; this function's
  // job is only to hand findTopMatchingListings' precise JS scorer a
  // reasonable pool, not to already be the precision layer itself.
  const MAX_FTS_TERMS = 15;
