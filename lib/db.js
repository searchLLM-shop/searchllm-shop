...
export async function bulkUpsertFeedListings(network, listings) {
  ...
  listings.forEach((l, i) => {
    const b = i * cols;
    valueRows.push(
      `($${b + 1}, $${b + 2}, $${b + 3}, $${b + 4}, $${b + 5}, $${b + 6}, $${b + 7}, $${b + 8}, 'pending', 'feed', $${b + 9}, now(), $${b + 10}, $${b + 11}, $${b + 12}, $${b + 13})`
    );
    params.push(
  ...
  const { rows } = await query(
    `INSERT INTO listings (brand, product, price, category, keywords, network, network_link, pitch, status, source, external_id, last_synced_at, regions, image_url, merchant_domain, discount)
