...
// confirmed / reversed. Points are FLAT (LOYALTY.POINTS.PURCHASE,
// redesigned 2026-08-25) — identical regardless of order size; the
// commission>0 condition is kept as the "a real, non-zero sale happened"
// filter, not as a scaling factor. Re-polls update in place (the click_id
// unique key), so pending → confirmed/reversed progresses cleanly. INR
// only, by design.
//
// The 250-point cap (reinstated 2026-08-25) only guards the INITIAL insert
// of a new click_id — a non-Plus member already at/over the cap earns
// nothing further from a NEW conversion. An already-inserted row's status
// transition (pending → confirmed/reversed) is NOT re-gated on the cap:
// those points were already provisionally counted, this is only correcting
// their status, not creating new points.
export async function syncLoyaltyLedger() {
  const { rowCount } = await query(
    ...
    [LOYALTY.POINTS.PURCHASE, LOYALTY.VOUCHER_UNLOCK_POINTS]
  );
  return rowCount;
}
...
export async function getRewardsSummary(userId) {
  ...
  return {
    ...
    confirmed,
    // Redesigned 2026-08-25: one unified total (search + click + purchase)
    // — totalPoints is the public name for what "confirmed" already was
    // structurally. For a non-Plus member this total stops growing at
    // VOUCHER_UNLOCK_POINTS (see creditSearchPoints/creditClickPoints/
    // syncLoyaltyLedger) — reaching it is the signal to upgrade, which both
    // lifts the ceiling and unlocks redemption.
    totalPoints: confirmed,
    canClaimVoucher: confirmed >= LOYALTY.VOUCHER_UNLOCK_POINTS,
...
// Credits search points to a REGISTERED user — every pick earns, UNLESS a
// non-Plus member is already at LOYALTY.VOUCHER_UNLOCK_POINTS (reinstated
// 2026-08-25: points stop for non-Plus at that ceiling; upgrading lifts it).
// One ledger row per search (source='search', immediately confirmed —
// there's no network to wait for). The trailing SELECT reads the pre-insert
// snapshot, so the CTE's points are added back explicitly — also correctly
// reports 0 earned (not the nominal rate) when the cap blocked the insert.
export async function creditSearchPoints(userId) {
  ...
  const { rows } = await query(
    `WITH ins AS (
       ...
       WHERE EXISTS (SELECT 1 FROM user_plans WHERE user_id = $1 AND plan = 'plus')
          OR COALESCE((SELECT SUM(points) FROM points_ledger
                       WHERE user_id = $1 AND status = 'confirmed'), 0) + $2 <= $3
       RETURNING points
     )
     SELECT
    ...
    [userId, per, LOYALTY.VOUCHER_UNLOCK_POINTS]
  );
...
// otherwise this would be a self-service points mint. Also capped at
// LOYALTY.VOUCHER_UNLOCK_POINTS for non-Plus members (reinstated
// 2026-08-25), same as creditSearchPoints. Returns points credited (0 on
// either the dedupe path or the cap).
export async function creditClickPoints(userId, listingId) {
  ...
  const { rowCount } = await query(
    `INSERT INTO points_ledger (user_id, points, status, source, note, listing_id)
     ...
     AND (
       EXISTS (SELECT 1 FROM user_plans WHERE user_id = $1 AND plan = 'plus')
       OR COALESCE((SELECT SUM(points) FROM points_ledger
                    WHERE user_id = $1 AND status = 'confirmed'), 0) + $2 <= $4
     )`,
    [userId, per, listingId, LOYALTY.VOUCHER_UNLOCK_POINTS]
  );
...
// Redesigned 2026-08-25 (explicit direction): guests earn all three point
// types same-day — search, click, AND purchase (flat 25, same as everyone,
// "we cannot do it differently because the commission data does not reach
// us") — but ALL of it vanishes at day's end unless they register that day.
// Note on purchases specifically: a network doesn't CONFIRM a sale for
// 30-90 days (Awin/vCommission, the only live networks), so
// conversion_status only flips to 'approved' weeks later in practice —
// meaning this purchases count will almost always be 0 for a real guest
// today, honestly. The query is correct and will start firing the moment a
// same-day-confirming channel (e.g. a direct-advertiser postback) is live;
// it is not a bug that it rarely shows anything under the current network
// mix, and the "vanishes if you don't register today" promise is still
// kept — there's simply nothing yet to vanish, most days.
export async function getGuestDaySummary(guestIdentity) {
  const { rows } = await query(
    `SELECT
       ...
       COALESCE((SELECT COUNT(*) FROM network_clicks
                 WHERE identity = $1 AND conversion_status = 'approved'
                   AND COALESCE(matched_at, created_at)::date = (now() AT TIME ZONE 'utc')::date), 0) AS purchases`,
    [guestIdentity]
  );
  const r = rows[0];
  ...
  const total = searches * LOYALTY.POINTS.GUEST_PER_PICK + clicks * LOYALTY.POINTS.CLICK + purchases * LOYALTY.POINTS.PURCHASE;
  return { searches, clicks, purchases, total };
}
...
// cookie, so TODAY's guest points (search, click, and any same-day-confirmed
// purchase) convert into real ledger points, once, per source. The note
// carries the date, and the WHERE makes each claim idempotent — re-calls
// the same day insert nothing more.
export async function claimGuestDayPoints(userId, guestIdentity) {
  ...
  const claimed = await Promise.all([
    ...
    claimOne("purchase", summary.purchases * LOYALTY.POINTS.PURCHASE),
  ]);
...
export async function getRewardsReport() {
  const [issuance, liability, byVoucher, byPlan, capped, dailyIssuance] = await Promise.all([
    query(
    ...
    // Members by plan — the direct readout of whether the points→upgrade
    // funnel is converting at all.
    query(
      `SELECT COALESCE(up.plan, 'free') AS plan, COUNT(*) AS members
       FROM loyalty_members m
       LEFT JOIN user_plans up ON up.user_id = m.user_id
       GROUP BY 1 ORDER BY 1`
    ),
    // Capped-out free members: at the 250-point ceiling, not yet Plus —
    // exactly the audience the homepage upgrade banner targets. High and
    // growing means the nudge isn't converting; worth watching over time.
    query(
      `SELECT COUNT(*) AS members FROM (
         SELECT pl.user_id
         FROM points_ledger pl
         JOIN loyalty_members m ON m.user_id = pl.user_id
         LEFT JOIN user_plans up ON up.user_id = pl.user_id
         WHERE COALESCE(up.plan, 'free') <> 'plus'
         GROUP BY pl.user_id
         HAVING COALESCE(SUM(pl.points) FILTER (WHERE pl.status = 'confirmed'), 0) >= $1
       ) capped_users`,
      [LOYALTY.VOUCHER_UNLOCK_POINTS]
    ),
  ...
  return {
    ...
    byPlan: byPlan.rows,
    cappedFreeMembers: Number(capped.rows[0]?.members || 0),
...
// The lifecycle engine (redesigned 2026-08-25, replacing the old free/guest
// query-and-click checkpoint ladder entirely — see LOYALTY.PLUS_QUERY_CYCLE_LIMIT
// in lib/constants.js for the full rationale). Free users and guests are
// NEVER blocked from research by this function any more; points are the
// only lever there, and they're purely a reward nudge (getRewardsSummary's
// canClaimVoucher), not a gate.
//
// The one remaining gate is Plus-only: a cycle = since the last confirmed
// purchase, the last Increase Usage payment, or the most recent Plus
// upgrade, whichever is latest. LOYALTY.PLUS_QUERY_CYCLE_LIMIT queries with
// ZERO purchases in that window → hard "recharge" gate (Increase Usage,
// ₹RECHARGE_PRICE_INR) until a purchase or payment resets it. Search counts
// come from points_ledger (source='search'), NOT usage_daily — Plus users
// get dailyPickLimit()=-1, so checkAndConsumeQuota (the only usage_daily
// writer) is never called for them; points_ledger IS written unconditionally
// per pick via creditSearchPoints, so it's the reliable counter here.
export async function getLifecycleStatus(identity) {
    ...
    `WITH anchor AS (
       SELECT GREATEST(
         ...
         COALESCE((SELECT MAX(created_at) FROM user_checkpoints
                   WHERE user_id = $1 AND kind = 'search' AND status = 'paid'), 'epoch'),
         COALESCE((SELECT updated_at FROM user_plans WHERE user_id = $1 AND plan = 'plus'), 'epoch')
       ) AS ts
     )
     SELECT
       (SELECT plan FROM user_plans WHERE user_id = $1) AS plan,
       (SELECT COUNT(*) FROM points_ledger
        WHERE user_id = $1 AND source = 'search' AND created_at > a.ts) AS searches_since,
       (SELECT COUNT(*) FROM network_clicks
        WHERE identity = $1 AND created_at > a.ts AND conversion_status = 'approved') AS purchases_since,
  ...
  const triggered = isPlus && searches >= LOYALTY.PLUS_QUERY_CYCLE_LIMIT && Number(r.purchases_since) === 0;
...
export async function getHeaderSnapshot(identity, userId) {
  ...
  return {
    plan: r.plan || "free",
    used: Number(r.used),
    totalPoints: Math.floor(Number(r.confirmed)),
...
export async function getReportSummary(days = 30, range = null) {
  const { from, to } = normalizeRange(days, range);
  const [totals, daily, topProducts, byNetwork, topUsers, activity, plans, inventory, sources, campaigns, revenue, inventoryByCountry, inventoryByCategory, rewardsReport, clicksReport, pwaStats, publicationStats] =
    await Promise.all([
      query(
        `SELECT
