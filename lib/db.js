...
export async function getLifecycleStatus(identity) {
  const [anchorRow, plan] = await Promise.all([
    query(
      `SELECT GREATEST(
    ...
    query(`SELECT plan FROM user_plans WHERE user_id = $1`, [identity]),
  ]);
  const anchor = anchorRow.rows[0].anchor;
  const isPlus = plan.rows[0]?.plan === "plus";
  ...
  const { rows } = await query(
    `SELECT
       ...
       (SELECT COUNT(*) FROM network_clicks WHERE identity = $1 AND created_at > $2) AS aff_clicks,
       (SELECT COUNT(*) FROM alt_clicks WHERE identity = $1 AND created_at > $2) AS alt_clicks,
       (SELECT COUNT(*) FROM user_checkpoints
        WHERE user_id = $1 AND kind = 'prompt' AND created_at > $2) AS prompts,
  ...
  return {
    ...
    // Plus: research never blocks; alternatives are withheld instead.
    suppressAlternatives: isPlus && triggered,
    // Guests/free: stage 1 = upgrade interstitial, stage 2 = hard recharge gate.
...
export async function getReportSummary(days = 30, range = null) {
  const { from, to } = normalizeRange(days, range);
  const [totals, daily, topProducts, byNetwork, topUsers, activity, plans, inventory, sources, campaigns, revenue, inventoryByCountry, inventoryByCategory, rewardsReport, clicksReport] =
    await Promise.all([
      query(
        `SELECT
