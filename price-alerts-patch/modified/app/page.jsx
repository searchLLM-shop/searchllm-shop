  ...
  return (
    <div style={{ minHeight: 600, display: "flex", flexDirection: "column", border: "0.5px solid var(--color-border-tertiary)", borderRadius: 16, overflow: "hidden", background: "var(--color-background-primary)" }}>
      ...
      <div className="sllm-tabs" style={{ display: "flex", borderBottom: "0.5px solid var(--color-border-tertiary)" }}>
        {["research", "saved", "watchlist", "rewards", ...(SHOW_BRANDS_FORM ? ["brands"] : []), ...(SHOW_ADVERTISERS ? ["advertise"] : []), ...(isAdminHint ? ["admin", "products", "queries", "performance", "answers", "reports", ...(SHOW_ADVERTISERS ? ["advertisers"] : [])] : [])].map((t) => (
          <button
            key={t}
          ...
          >
            ...
            {t === "research"
              ...
              ? "Review queue"
              : t}
            {t === "watchlist" && watchlistUnseen > 0 && (
              <span style={{ marginLeft: 6, background: "#D85A30", color: "#fff", borderRadius: 10, fontSize: 10, fontWeight: 600, padding: "1px 6px" }}>
                {watchlistUnseen}
              </span>
            )}
          </button>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: 20 }}>
        ...
        {activeTab === "watchlist" && <PriceAlerts onMarkSeen={() => setWatchlistUnseen(0)} />}
